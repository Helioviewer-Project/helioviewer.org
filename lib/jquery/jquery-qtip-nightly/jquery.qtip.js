...
(function($)
{
	...
	*/
	$(function()
	{
		...
		$(window).bind('resize.qtip scroll.qtip', function(event)
		{
			$(':not(.qtip.ui-tooltip)').each(function()
			{
				...
				// Dertermine current tooltip size
		...
		$(document).bind('mousedown.qtip', function(event)
		{
			if($(event.target).parents('div.qtip').length === 0)
			{
				$(':not(.qtip.ui-tooltip)').each(function()
				{
					...
					if(api.options.hide.when.event === 'unfocus' && $(event.target).add(api.elements.target).length > 1 &&
					api.elements.tooltip.is(':visible') && !api.elements.tooltip.hasClass('ui-state-disabled')){
						api.hide(event);
					}
				});
			}
	...
	function sanitizeOptions(opts)
	{
		...
		if(opts.content !== undefined)
		{
			...
			if(opts.content.title && typeof opts.content.title !== 'object'){ opts.content.title = { text: opts.content.title }; }
		}
	...
	*/
		...
		*/
		...
		function calculate(detail)
		{
			var show = (!self.elements.tooltip.is(':visible')) ? true : false,
				returned,
				tipAdjust = self.plugins.tip ? (self.plugins.tip.corner.precedance === 'x') ? ['width', 'left'] : ['height', 'top'] : 0;
			...
			switch(detail)
			{
				case 'dimensions':
					...
					if(tipAdjust) {
						returned[ tipAdjust[0] ] += self.elements.tip[ tipAdjust[0] ]();
					}
				break;
				...
				case 'position':
					...
					// Account for tip if present
					if(tipAdjust) {
						returned[ tipAdjust[1] ] -= self.elements.tip[ tipAdjust[0] ]();
					}
				break;
			}
		...
		{
			var elems = this.elements, max, min;
			...
			max = parseInt(elems.tooltip.css('max-width'), 10) || 0;
			min = parseInt(elems.tooltip.css('min-width'), 10) || 0;
		...
		function updateStyle()
		{
			...
			if(options.style.border){ self.elements.tooltip.qcorner('update'); }
		}
		...
		function assignEvents(show, hide, tooltip)
		{
			var prepend = '.qtip-'+id,
				...
				events = { show: options.show.when.event, hide: options.hide.when.event };
			...
			function showMethod(event)
			{
				...
				if(events.hide === 'inactive'){ targets.show.trigger('qtip-'+id+'-inactive'); }
			...
			function hideMethod(event)
			{
				...
				if(events.hide)
				{
					...
					if(hideOpts.fixed && events.hide.search(/mouse(out|leave)/i) !== -1 && $(event.relatedTarget).parents('.qtip.ui-tooltip').length > 0)
					{
					...
					self.timers.hide = setTimeout(function(){ self.hide(event); }, hideOpts.delay);
				}
			}
			...
			if(events.hide === 'inactive')
			{
				...
				$($.fn.qtip.inactiveEvents).each(function(){ targets.hide.add(self.elements.content).bind(this+prepend+'-inactive', inactiveMethod); });
			}

			// Both events and targets are identical, apply events using a toggle
			if((events.show === events.hide && events.hide !== 'inactive' && $(targets.show).add(targets.hide).length === 1) || events.hide === 'unfocus')
			{
				targets.show.bind(events.show+prepend, function(event)
				{
			...
			{
				// Bind show event
				targets.show.bind(events.show+prepend, showMethod);

				// If the hide event is not 'inactive', bind the hide method
				if(events.hide !== 'inactive'){ targets.hide.bind(events.hide+prepend, hideMethod); }
				if(events.show === 'mouseover'){ targets.hide.bind('mouseout'+prepend, hideMethod); }
			}
			...
			if(options.position.target === 'mouse')
			{
				targets.show.bind('mousemove'+prepend, function(event)
				{
					...
					cacheMouse({ left: event.pageX, top: event.pageY });

					// Update the tooltip position only if the tooltip is visible and adjustment is enabled
					if(!targets.tooltip.hasClass('ui-state-disabled') && options.position.adjust.mouse && targets.tooltip.is(':visible')) {
		...
		function unassignEvents(show, hide, tooltip)
		{
			...
			if(self.rendered)
			{
				...
				targets.show.unbind('mousemove'+prepend)
					...
					.unbind(events.show+prepend)
					.unbind('qtip-'+id+'-inactive');

				// Remove hide events
				targets.hide.unbind('mouseout'+prepend).unbind(events.hide+prepend);
				$($.fn.qtip.inactiveEvents).each(function(){
					targets.hide.add(tooltip ? self.elements.content : null).unbind(this+prepend+'-inactive');
				});

				// Remove tooltip events
				targets.tooltip.unbind(events.hide+prepend).unbind('mouseover'+prepend);
			}
		...
		*/
		$.extend(self, {
			...
			render: function(show)
			{
				...
				if(typeof options.style.border === 'object' && $.fn.qcorner)
				{
					// Make tooltip accessible but hidden
					self.elements.tooltip.addClass('ui-tooltip-accessible');

					// Setup border width
					self.elements[ options.style.border.expand ? 'wrapper' : 'tooltip' ].css({ borderWidth: options.style.border.border.width });

					// Apply the border
					options.style.border = self.elements.tooltip.qcorner( $.extend(true, options.style.border, { wrap: false }) );

					// Revert accessiblity class
					self.elements.tooltip.removeClass('ui-tooltip-accessible');

					// Adjust element references
					self.elements.content = self.elements.wrapper.find('.ui-tooltip-content');
				}
			...
			set: function(notation, value)
			{
				var option = convertNotation(notation.toLowerCase()),
					...
					checks = {
						// Content checks
						'^content.text': function(){ self.updateContent(); },
						'^content.title.text': function(){ updateTitle(options.content.title.text); },
			...
			toggle: function(state, event)
			{
				...
				// Call API method and if return value is false, halt
				if(tooltip.trigger('tooltip'+type, [self.hash(), 90]) === false){ return self; }
				...
				if(state) {
					...
					self.updatePosition(event, false);  // Update tooltip position
			...
			updatePosition: function(event, animate)
			{
				...
				var target = (event && options.position.target === 'mouse') ? event : $(options.position.target),
					tooltip = self.elements.tooltip,
					...
					position,
					threshold = options.position.adjust.effect.threshold,
					newCorner,
					tip,
					adapt = {
						fit: {
				...
				if(target.is('area') && $.fn.qtip.plugins.areaDetails) {
					...
					position = { top: target.pageY, left: target.pageX };
				}
				...
				position.left += posOptions.adjust.x + (at.x === 'right' ? targetWidth : at.x === 'center' ? targetWidth / 2 : 0);
				position.top += posOptions.adjust.y + (at.y === 'bottom' ? targetHeight : at.y === 'center' ? targetHeight / 2 : 0);

				// Adjust position relative to tooltip
				position.left -= my.x === 'right' ? elemWidth : my.x === 'center' ? elemWidth / 2 : 0;
				position.top -= my.y === 'bottom' ? elemHeight : my.y === 'center' ? elemHeight / 2 : 0;
				...
				if(self.elements.tip)
				{
					// Retrieve tip information
					tip = self.plugins.tip;

					// Determine which properties need adjusting
					position.adjust.which = (tip.corner.precedance === 'x') ? ['left', 'width'] : ['top', 'height'];

					// Adjust positioning to account for tip dimensions
					position[ position.adjust.which[0] ] +=
						(tip.corner[ tip.corner.precedance ] === position.adjust.which[0] ? 1 : -1) * (tip.size[ position.adjust.which[1] ] - Number($.browser.msie));
				}

				// Call API method and if return value is false, halt
				if(tooltip.trigger('tooltipmove', [self.hash(), position]) === false){ return self; }

				// Determine if animation is to take place
				if(posOptions.target === 'mouse' && posOptions.adjust.mouse){ animate = false; }
				if(animate === undefined)
				{
					if(Math.max(threshold, position.adjust.left, position.adjust.top) !== threshold){ animate = true; }
					else{ animate = false; }
				}

				// Update tooltip position
				if(animate && posOptions.adjust.effect !== false) {
					// Use custom function if provided
					if($.isFunction(posOptions.adjust.effect)) {
					...
					else if(posOptions.adjust.effect === true) {
						tooltip.animate(position, 110, 'swing');
					}
				}
				else {
	...
	{
		var content, command, opts;

		// Parse command into string
		command = String(options).toLowerCase();
