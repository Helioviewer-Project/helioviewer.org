		...
		function assignEvents(show, hide, tooltip)
		{
			...
			function hideMethod(event)
			{
				...
				if(events.hide)
				{
					...
					if(options.hide.fixed && (/mouse(out|leave)/i).test(events.hide) && $(event.relatedTarget).parents('.qtip.ui-tooltip').length > 0)
					{
			...
			if(events.hide === 'inactive')
			{
				...
				$($.fn.qtip.inactiveEvents).each(function(){ targets.hide.add(self.elements.tooltip).bind(this+prepend+'-inactive', inactiveMethod); });
			}
