...
var TileLayer = Layer.extend( 
    ...
    reset: function (zoomLevelChanged) {
        ...
        scaleOffset  = this.scale / currentScale;
	...
	getTile: function (x, y) {
        ...
        img.error(function (e) {
            ...
            if (self.controller.backupEnabled) {
                ...
                $(this).attr("src", self.getTileURL(self.controller.backupServer, x, y, zoom, filepath, filename, ts));
		...
		// Load tile
		img.attr("src", this.getTileURL(this.server, x, y, zoom, filepath, filename, ts));
        
        console.log(this.getTileURL(this.server, x, y, zoom, filepath, filename, ts));
	 ...
	 */
	getTileURL: function (server, x, y, zoom, filepath, filename, ts) {
		var uri, format;
		
		uri    = filepath + "/" + filename;
		format = (layeringOrder === 1 ? "jpg" : "png");
		
		return server + '?action=getTile&x=' + x + '&y=' + y + '&zoom=' + zoom + '&ts=' + ts + '&format=' + format + '&uri=' + uri;
	},
