...
var TileLayer = Layer.extend( 
    ...
    reset: function (zoomLevelChanged) {
        var currentScale, scaleFactor, offsetX, offsetY;
        
        // Start loading indicator
        //$('#loading').show();
        
        currentScale = this.controller.baseScale * Math.pow(2, this.viewport.zoomLevel - this.controller.baseZoom);
        scaleFactor  = this.scale / currentScale;
        
        // Update relevant dimensions
        this.relWidth  = this.width  * scaleFactor;
        this.relHeight = this.height * scaleFactor;
        
        // Offset image
        offsetX = ((this.width  / 2) - this.x) * scaleFactor;
        offsetY = ((this.height / 2) - this.y) * scaleFactor;
        ...
        /** TODO 2009/09/11: Compute minZoom when image is loaded (need scale). */
    ...
    /**
     * @description Sets up image properties that are not dependent on the specfic image, but only on the type (source) of the image.
     * 
     ...
     * 
     * IE: CSS opacities do not behave properly with absolutely positioned elements. Opacity is therefor set at tile-level.
     */
     ...
     */
    getTileURL: function (server, x, y) {
        var uri, zoomLevel, format, src;
        
        uri    = this.filepath + "/" + this.filename;        
        format = (this.layeringOrder === 1 ? "jpg" : "png");
        zoomLevel = this.viewport.zoomLevel;
        
        src  = server + '?action=getTile&uri=' + uri + '&x=' + x + '&y=' + y + '&zoom=' + zoomLevel + '&ts=' + this.tileSize;
        src += '&jp2Width=' + this.width + '&jp2Height=' + this.height + '&jp2Scale=' + this.scale + '&format=' + format;
        src += '&obs=' + this.observatory + '&inst=' + this.instrument + '&det=' + this.detector + '&meas=' + this.measurement;
