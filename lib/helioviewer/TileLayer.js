 ...
 * 
 */
/*global Class, Layer, Ajax, Event, $, jQuery, Element, Image */
var TileLayer = Class.create(Layer, 
	...
	initialize: function (viewport, options) {
		...
		$("moving-container").appendChild(this.domNode);

        jQuery("#helioviewer-viewport").bind('viewport-move', function (e) {
            self.viewportMove();
        });
...
//    	console.log("relative Width & Height: " + this.relWidth + " , " + this.relHeight + "\nWidth & Height: " + this.width + ", " + this.height);
	...
	loadClosestImage: function () {
		var time = this.viewport.controller.date.getTime(),
			processResponse, xhr;
	...
	getTile: function (x, y) {
        ...
        // IE
        if (!jQuery.support.opacity) {
			img.setOpacity(this.opacity / 100);
		}
	...
	loadJP2Dimensions: function () {
		var xhr, dimensions, processResponse;	
		processResponse = function (transport) {
			dimensions = transport.responseText.split("x");

			this.jp2Width 	= dimensions[0];
			this.jp2Height 	= dimensions[1];
		};
		...
		xhr = new Ajax.Request('api/index.php', {
			...
			onComplete: processResponse.bind(this)
		});
	},
