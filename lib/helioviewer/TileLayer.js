...
var TileLayer = Class.create(Layer, {
	...
	reset: function (visible) {
		...
		var max = Math.sqrt(this.getNumTiles()) / 2;
	...
	viewportMove: function (position) {
		...
		for (var i = -max; i <= max - 1; i++) {
			for (var j = -max; j <= max + 1; j++) {
	 ...
	 * @description Returns the number of tiles required to display images for the TileLayer's associated detector,
	 *              given the specified zoom-level. Varies from detector to detector.
	 */
	getNumTiles: function () {
		var zoom = this.viewport.zoomLevel;
		var diff = this.lowestRegularZoomLevel - zoom;
		
		if (diff > 0) {
			return Math.pow(4, 1 + diff);
		}
		else {
			return 4;
		}
