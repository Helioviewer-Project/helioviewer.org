			...
			if (!self.active) {
				...
				xhr = new Ajax.Request(self.url, {
					...
					parameters: {
	                	...
	                	layers:    "SOH_EIT_EIT_304,SOH_LAS_0C2_0WL",
...
var UserSettings = Class.create(
	...
	initialize: function (controller) {
		 ...
		 */
		this._DEFAULTS = $H({
			'obs-date'			: getUTCTimestamp(this.controller.defaultObsTime), //1065312000000,
	...
	toggleMouseCoords: function () {
		...
		if (this.viewport.mouseCoords !== "disabled") {
			...
			updateMouseCoords = function (e) {
				...
				//scale
				scale = self.viewport.controller.baseScale * Math.pow(2, self.viewport.zoomLevel - self.viewport.controller.baseZoom);
