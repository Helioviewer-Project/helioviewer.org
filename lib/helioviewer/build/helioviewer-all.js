...
var Helioviewer = Class.create(
	...
	initialize: function (viewportId, api, view, defaults) {
		...
		// Starting date
		this.date = new Date(this.userSettings.get('date'));
	...
	loadUserSettings: function () {
		...
		if (this.load["img-scale"]) {
			this.userSettings.set('zoomLevel', this.scaleToZoomLevel(this.load["img-scale"])); //(parseInt(this.load["img-scale"], 10)));
		}
	...
	_createScreenshotButton: function() {
		...
		this.button.click(function() {
			...
			if(ok) {
				...
				self.takeScreenshot(visibleCoords, self);
			}
		});
	},

	takeScreenshot: function(visibleCoords, self) {
		...
		var layers 		= self.getViewportInformation(visibleCoords), layerNames, layerOpacities, layerXRanges, layerYRanges, hcOffset;
	...
	_initEvents: function () {
        ...
        jQuery('#jhelioviewer-button').click(function () {
            //<a href="http://www.jhelioviewer.org/" style="line-height: 1.6em" target="_blank">JHV</a>
	...
	setDate: function (date) {
		...
		this.userSettings.set('date', parseInt(ts, 10));
	...
	getViewportInformation: function(visibleCoords) {
		var layers, hcOffset, tileLayers;
		
		// Get the top, left, bottom, and right coordinates of what is currently visible in the viewport			
		viewportCoords = this.viewport.getHCViewportPixelCoords(); 
		centerX = viewportCoords.left + (viewportCoords.right  - viewportCoords.left) / 2;
		centerY = viewportCoords.top  + (viewportCoords.bottom - viewportCoords.top) / 2;
    		...
    		jQuery('#select-image-dialog').load("dialogs/select-region.php").dialog({
				...
				buttons: 	{
					...
					"Build Movie": function() { 
						self.cleanup();
						helioviewer.movieBuilder.buildMovie(coords, helioviewer.movieBuilder);
					},
					"Take Screenshot": function() {
						self.cleanup();
						helioviewer.takeScreenshot(coords, helioviewer);
					}
				}
    		});
...
var LayerManager = Class.create(UIElement,
	...
	addNewLayer: function () {
		var priorityQueue, currentLayers, p, api, serverNum, layer, defaultLayer = "SOH_EIT_EIT_171";
		priorityQueue = $A([
			"SOH_EIT_EIT_304", "SOH_LAS_0C2_0WL", "SOH_LAS_0C3_0WL", "SOH_LAS_0C2_0WL", "SOH_MDI_MDI_mag", "SOH_MDI_MDI_int", "SOH_EIT_EIT_171", "SOH_EIT_EIT_284", "SOH_EIT_EIT_195"
		]);
		
		// current layers in above form
		currentLayers = $A([]);
		this.tileLayers().each(function (l) {
			currentLayers.push(l.observatory + "_" + l.instrument + "_" + l.detector + "_" + l.measurement);
		});
        ...
        layer = this.controller.parseLayerString(p);
    ...
    /**
     * @description Generates a string representation of a given set of layer parameters, e.g. "SOH_LAS_0C2_0WL".
     * @param {Object} A tilelayer
     * @returns {String} A string representation of the given layer
 	 * @see Helioviewer#parseLayerString
     */
    generateLayerString: function (layer) {
        return layer.observatory + "_" + layer.instrument + "_" + layer.detector + "_" + layer.measurement;
    },
...
var MessageConsole = Class.create(UIElement ,
    ...
    info: function (msg) {
       	this.console.update(new Element('p', {style: 'color: red; font-weight: bold;'}).insert(msg));
		var self = this,
        	trash = new Effect.Appear(this.console);
	...
	progress: function(msg) {
		this.console.update(new Element('p', {style: 'color: black;'}).insert(msg));
		var self = this,
			trash = new Effect.Appear(this.console);
		window.setTimeout(function() {
			self.console
...
var MovieBuilder = Class.create(UIElement, {
	...
	initialize: function (controller) {
		...
		this.button.click(function () {			
			var helioviewer = self.controller, ok = true, visibleCoords;

			if (self.building) {
				ok = confirm("Warning: This will re-start the movie building process. Do you want to continue?"); //helioviewer.messageConsole.info("Your movie is already being built. The link to view the movie will appear here shortly.");
			}
		
			if (ok) {
				visibleCoords = helioviewer.viewport.getHCViewportPixelCoords(); 
				self.buildMovie(visibleCoords, self);
			}
		});
	},

	buildMovie: function (visibleCoords, self) {	
		var helioviewer = self.controller, zoomLevel, startTime, hqformat, layers = $A(), layerNames = $A(), layerOpacities = $A(), layerXRanges = $A(), layerYRanges = $A(), xhr, hcOffset;
		self.building = true;
...
//		this.updateProgress(timeout);	
		...
		xhr = new Ajax.Request(this.url, {
			...
			onComplete: function (transport) {
				...
				url = 'api/index.php?action=playMovie&format=' + hqformat + '&url=' + transport.responseJSON;
				self.building = false;
...
var TileLayer = Class.create(Layer, 
	...
	getTile: function (x, y) {
		...
		// Load tile
		img.src = this.tileAPI + '?action=getTile&x=' + x + '&y=' + y + '&zoom=' + zoom + '&uri=' + this.uri + '&ts=' + ts;
...
var TileLayerAccordion = Class.create(Layer,
    ...
    _removeTooltips: function (id) {
        ...
        ids.each(function (id) {
			jQuery(id).unbind();
            //jQuery(id).unbind("mouseover").unbind("mouseout").unbind("click");
		});
    },
...
var UserSettings = Class.create(
	...
	_loadDefaults: function () {
        ...
        if (this.controller.support.localStorage) {
            ...
            if (this.controller.support.nativeJSON)
...
var ZoomControl = Class.create(UIElement,
	 ...
	 */
    changed: function (v) {
    	this.fire('zoom-slider-change', v);
    }
});