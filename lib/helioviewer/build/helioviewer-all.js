...
var EventLayerAccordion = Class.create(Layer, {
	...
	getEventCatalogs: function () {
		...
		jQuery.getJSON(this.viewport.controller.eventAPI, { action: "getEventCatalogs" } ,function(catalogs) {
	...
	_setupUI: function () {
		...
		addLayerBtn.click(function() {
			...
			Event.observe($(tmpId).select('button').first(), 'click', function(e) {
				...
				self.domNode.dynaccordion('removeSection', {id: tmpId});
				self.viewport.controller.layerManager.addLayer(new EventLayer(self.viewport, {catalog: select.value, eventAccordion: self}));
			});
		});
	},
...
var Helioviewer = Class.create({
	...
	initialize: function (options) {
		...
		// Starting date
		this.date = new Date(parseInt(this.userSettings.get('obs-date')));
	...
	loadUserSettings: function () {
		...
		if (this.load["obs-date"]) {
			this.userSettings.set('obs-date', parseInt(this.load["obs-date"])*1000);
		}
		
		if (this.load["img-scale"]) {
			this.userSettings.set('zoom-level', this.scaleToZoomLevel(parseInt(this.load["img-scale"])));
		}
		
		if (this.load["layers"]) {
			layers = new Array();
			$A(this.load["layers"]).each(function (layer) {
				layers.push({ tileAPI: "api/getTile.php", observatory: layer.substr(0,3), instrument: layer.substr(3,3), detector: layer.substr(6,3), measurement: layer.substr(9,3) });
			});
...
var IconPicker = Class.create({
	...
	_buildIconList: function () {
		...
		var closeBtn = jQuery('<br><div style="text-align: right"><a class="event-url" href="#" style="margin-right: 2px;">[Close]</a></div>').click(function(){jQuery('#event-icon-menu').fadeOut()});
...
var LayerManager = Class.create(UIElement, {
	...
	getMaxDimensions: function () {
		...
		//Debug.output("Max dimensions: " + maxWidth + ", " + maxHeight);
	...
	refreshSavedTileLayers: function () {
		Debug.output("refreshSavedTileLayers");
		var tilelayers = [];
...
var UserSettings = Class.create({
	...
	_loadDefaults: function () {
		...
		this.set('tile-layers', [
			{ tileAPI: "api/getTile.php", observatory: 'SOH', instrument: 'EIT', detector: 'EIT', measurement: '195' }
		]);
		
		// { tileAPI: "api/getTile.php", observatory: 'SOH', instrument: 'LAS', detector: '0C2', measurement: '0WL' }
		// { tileAPI: "api/getTile.php", observatory: 'SOH', instrument: 'MDI', detector: 'MDI', measurement: 'mag' };
...
var ViewportHandlers = Class.create({
	...
	toggleMouseCoords: function () {
		...
		// Case 3: Polar Coords -> Disabled
			...
			Debug.output("Polar Coords -> Disabled");
		}
