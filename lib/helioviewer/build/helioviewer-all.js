...
var Helioviewer = Class.create({
	...
	initViewports: function () {
		this.viewports = $A([
			new Viewport(this, { id: this.viewportId, zoomLevel: this.defaultZoomLevel, prefetch: this.defaultPrefetchSize, debug: true })
		]);
...
var TileLayer = Class.create(Layer, {
	...
	reset: function (visible) {
		...
		// Limit to 64 tiles for scalability
		max = Math.min(4, max);
		
		Debug.output("TL Max: " + max);
