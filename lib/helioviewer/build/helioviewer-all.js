	...
	_setupEventHandlers: function (layer) {
		...
		var showFITS = function () {
			...
			if (jQuery("#" + dialogId).length == 0) {
				...
				// Ajax Request
				var xhr = new Ajax.Request("api/getFitsHeader.php", {
					method: 'get',
					onSuccess: processResponse.bind(this),
					parameters: {
...
var UserSettings = Class.create({
	...
	_loadDefaults: function () {
		...
		// Date
		this.set('obs-date', 1065312000000); // Milliseconds since Oct 05, 2003, UTC
...
var Viewport = Class.create(UIElement, {
	...
	*/ 
	...
	zoomTo: function (zoomLevel) {
		...
		// update sandbox
		var d = this.controller.layerManager.getMaxDimensions();
		this.updateSandbox(d.width, d.height);
