...
var LayerManager = Class.create(UIElement, {
    ...
    initialize: function (viewport, htmlContainer) {
        ...
        this.viewport.controller.observe(this, 'newToolTip', this.viewport.controller.handlers.newToolTip);
        ...
        this.viewport.layers.each (function (layer) {
            if (parseInt(layer.opacityGroupId) == parseInt(self.opacityGroupId)) {
                ...
                if (counter > 1) {
                    ...
                    layer.domNode.setStyle({'filter': 'alpha(opacity=' + (opacity * 100) + ')', width: '256px'})
...
var Viewport = Class.create(UIElement, {
    ...
    initialize: function (controller, options) {
        ...
        Event.observe(this.movingContainer, 'dblclick', function (e) {
            ...
            if ((self.zoomLevel > self.controller.minZoomLevel) && (self.zoomLevel < self.controller.maxZoomLevel)) {
                if (e.isLeftClick()) {
                    //Compute offset from top-left of browser viewport
                    var xOffset = $('left-col').getDimensions().width + Math.round(0.03 * self.outerNode.getDimensions().width) + 2;
                    var yOffset = $(self.headerId).getDimensions().height + Math.round(0.03 * self.outerNode.getDimensions().height) + 3;
                    
                    //Determine the location of 0,0 in the viewport
                    var origin = {
                        x: Math.round(self.domNode.getDimensions().width / 2),
                        y: Math.round(self.domNode.getDimensions().height / 2)
                    }
                    ...
                    //Initiate zoom
                    e.shiftKey ? self.controller.zoomControl.zoomButtonClicked(1) : self.controller.zoomControl.zoomButtonClicked(-1);
                }         
            }   
        });
        this.resize();
    ...
    /**
     * @method center Center the viewport.
     */
    center: function () {
    ...
    /**
     * @method moveTo Move the viewport focus to a new location.
     * @param {int} x-value
     * @param {int} y-value
     */    
    moveTo: function (x, y) {
    ...
    /**
     * @method moveBy Shift in the viewport location.
     * @param {int} x-value
     * @param {int} y-value
     */   
    moveBy: function (x, y) {
