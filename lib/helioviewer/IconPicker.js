...
var IconPicker = Class.create(
	...
	_buildIconList: function () {
		...
		i = 1;
		jQuery.each(this.AVAILABLE_ICON_COLORS, function () {
            var color = this;
			jQuery.each(self.AVAILABLE_ICON_SHAPES, function () {
                var icon, shape = this;
				
                icon = jQuery('<img class="event-icon-menu-icon" src="images/events/small-' + color + "-" + shape + '.png" alt="' + color + '-' + shape + '">');
				icon.click(function () {
