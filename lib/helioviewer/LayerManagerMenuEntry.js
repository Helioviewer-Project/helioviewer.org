...
var LayerManagerMenuEntry = Class.create(UIElement, {
    ...
    displayTileLayerOptions: function () {
        //this.header =  this.createMenuRowHeader(this.layer);
        //this.content = this.createMenuRowContent(this.layer);
        ...
        if (!this.header) {
            ...
            this.header.update (this.createMenuRowHeader(this.layer));
	        this.content.update(this.createMenuRowContent(this.layer));
        }
    ...
    */
    displayNewLayerOptions: function () {
        ...
        var self = this;
        var ajax = new Ajax.Request ('getInstruments.php', {
            onSuccess: function (transport) {
                var instruments = transport.responseJSON;
                
                // Create instrument HTML Element
                var inst = new Element('select');
                inst.length = 0;
        
                // Populate list of available instruments (IE-Safe method)
                for (var i = 0; i < instruments.length; i++) {
                    var opt = new Option(instruments[i].instrument);
                    opt.value = instruments[i].instrument;
                    inst.options[inst.options.length] = opt;
                }
                
                // Place HTML Element
                var div = Builder.node('div', {style: 'color: white;'}, ['Select Instrument: ', inst])
                self.content.update(div);
                
                // Event-handler
                Event.observe(inst, 'change', (function (instruments, e) {
                    
                    // Get info for chosen instrument
                    var chosenInstrument = null;
                    for (var i = 0; i < instruments.length; i++) {
                        if (instruments[i].instrument === e.target.value) {
                            chosenInstrument = instruments[i];
                        }
                    }
                    self.layermanager.fire('newLayer', {menuEntry: self, instrument: chosenInstrument})

                }).bind(self, instruments));
            },
            onFailure: function () {
                throw new Error('AJAX CALL FAILED: LayerManagerMenuEntry -> displayNewLayerOptions');
            }
        });
    },
    ...
    */
    createMenuRowHeader: function (layer) {
        ...
        var rightSide = Builder.node('div', {style: 'text-align: right; float: left; width:70%;'}, [timestamp, " |", this.visibilityBtn, this.removeBtn]);
    ...
    createMenuRowContent: function (layer) {
    	...
    	if (layer.instrument === "EIT") {
    		...
    		body.appendChild(Builder.node('div', {style: 'height: 24px; padding: 4px;', id: 'measurement-row-' + layer.id}, [wlLeft, wlRight]));
    	}
