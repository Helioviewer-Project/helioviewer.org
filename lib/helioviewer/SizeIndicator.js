 ...
 */
 /*global document, Class, $, $A, $H, Element, Event, Effect, Debug, Draggable, TileLayerProvider, DomNodeCache, OverlayCollection, numberOfTileLayers, numberOfMarkerLayers, AjaxRequestWrapper, Option */
var SizeIndicator = Class.create();
 
SizeIndicator.prototype = {
	/**
	 * @constructor		
	 ...
	 */
	initialize: function (elementId, tileSize, options) {
		// Default options
	...
	createElements: function () {
		...
		var imgL = new Element('img', {'src': this.imgLsrc, 'alt': 'Increase the size of the size-indacator.'}); 	
	    var imgR = new Element('img', {'src': this.imgRsrc, 'alt': 'Increase the size of the size-indacator.'});
		...
		//Make draggable
		var trash = new Draggable(this.domNode);
	},
	...
	displaySize: function () {
		var convertedZoom = this.zoomLevel;
		...
		var widthKm = (this.width * SizeIndicator.sunRadiusKm) / (this.sunRadius * this.tileSize * Math.pow(2, convertedZoom));
	...
	onZoomLevelChange: function (type, args) {
		...
		//Debug.output('[SizeIndicator->onZoom] this.zoomLevel = '+ this.zoomLevel);
	...
	onImageChange: function (type, args) {
		...
		if (sunImage.sunRadius !== this.sunRadius) {
			this.width = sunImage.sunRadius * this.tileSize * 2;
	...
	startResizingHandler: function (e, dir) {
		...
		this.startLeft = parseInt(this.domNode.style.left, 10);
	...
	mouseUpHandler: function (e) {
		...
		if (this.resizing) {
			...
			pos = Element.positionedOffset(this.domNode);
			this.position = { left: pos[0], top: pos[1] };
