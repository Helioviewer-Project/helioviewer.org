...
var ImageSelectTool = Class.create(UIElement, {
	initialize: function (controller) {
		...
		this.button.click(function () {
			var fullscreen = jQuery("#fullscreen-btn"), width, height, imgContainer, transImg, helioviewer, dialog;
...
/*			onSelectChange:	function (img, selection) {
					...
					onFinish: function () {
						jQuery('#take-screenshot-button').click(function () {
							...
							setTimeout(function (thisObj) {
								thisObj.takeScreenshot(helioviewer, coords);
							}, 1000, self);
	...
	takeScreenshot: function (helioviewer, visibleCoords) {
		var zoomLevel, obsDate, layerNames = $A(), xhr, hcOffset, imgWidth, imgHeight, vpDimensions, vpWidth, vpHeight, sbWidth, sbHeight, url;
		this.mediaSettings.getDefaultInformation(this.mediaSettings, visibleCoords);

		zoomLevel = this.mediaSettings.zoomLevel; 
		obsDate   = this.mediaSettings.startTime;
		helioviewer.building = true;

		imgWidth  = this.mediaSettings.width; 
		imgHeight = this.mediaSettings.height; 

		// Get all of the layer information
		//layers 	= helioviewer.getViewportInformation(visibleCoords);
		layerNames 		= this.mediaSettings.layers; //layers.layerNames;
		hcOffset 		= this.mediaSettings.hcOffset; //layers.hcOffset;
		...
		xhr = new Ajax.Request('api/index.php?', {
			...
			parameters: {
				...
				obsDate: 	obsDate,
				zoomLevel: 	zoomLevel,
				edges: 		this.mediaSettings.edgeEnhance,
				sharpen: 	this.mediaSettings.sharpen,
	...
	cleanup: function () {
		Shadowbox.close();
		var dialogDiv = jQuery('#select-image-dialog'), fullscreen;
