	...
	initialize: function (controller) {
		...
		this.button.click(function () {
			...
			// Otherwise, turn it on.
				...
				jQuery("#fullscreen-btn").addClass('requests-disabled');
				self.active = true;
				 ...
				 */
                trans = '<img id="transparent-image" src="images/transparent_512.png" alt="" width="' + width + '" height="' + height + '" />';
                trans.css({'position': 'relative', 'cursor': 'crosshair', 'z-index': 5});
                helioviewer.viewport.domNode.append(trans);
				 ...
				 */
                imgContainer = jQuery('body').append('<div id="imgContainer"></div>');
...
/*			onSelectChange:	function (img, selection) {
	...
	cleanup: function () {
		...
		jQuery('#imgContainer, #transparent-image').remove();
		
		this.active = false;
		jQuery("#fullscreen-btn").removeClass('requests-disabled');
	}
});
