...
var ImageSelectTool = Class.create(
	 ...
	 */
	selectArea: function () {
		var helioviewer = this.controller, self = this, coords = $A(), visibleCoords = $A();
...
/*			onSelectChange:	function (img, selection) {
	...
	createDialog: function (coords) {
		var helioviewer = this.controller, self = this;
			...
			options:	{
				onFinish: function () {
					jQuery('#take-screenshot-button').click(function () {
						self.cleanup();
						helioviewer.screenshotBuilder.takeScreenshot(coords);														
					});
					...
					jQuery('#build-movie-button').click(function () {
						...
						setTimeout(function () {
							helioviewer.movieBuilder.checkMovieLayers(coords);								
						}, 1000);				
					});
				}			
			},
