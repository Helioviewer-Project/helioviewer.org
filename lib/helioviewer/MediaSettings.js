...
var MediaSettings = Class.create(
	 ...
	 * Settings that are not affected by a change in the viewport: url, numFrames, frameRate, sharpen, edgeEnhance, 
	 * format, showImgIfGap, emailUser, emailAddress, quality.
	 * Settings that ARE affected when the viewport changes: timeStep, startTime/EndTime, layers, filename.
	 * TimeStep currently sticks with whatever the user picks. May need to change this.
	 * startTime/endTime are determined by the viewport settings, not the user's. 
	 * Layers are pulled from the viewport. Whether they are checked or not depends on what the user put though.
