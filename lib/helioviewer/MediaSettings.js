	...
	getImageInformation: function (visibleCoords, vpWidth, vpHeight) {
		...
		$.each(helioviewer.tileLayers, function () {
			...
			if (this.visible) {
				...
				if (this.detector.toString() === "C2" && this.insideCircle(216, jp2Width / 2, left, top, width, height)) { 
					width = -1;
				}
				// 104 is the radius of the transparent circle in a C3 image.
				else if (this.detector.toString() === "C3" && this.insideCircle(104, jp2Width / 2, left, top, width, height)) { 
					width = -1;
				}
