...
var MediaSettings = Class.create(
	 ...
	 * Settings that are not affected by a change in the viewport: timeStep, url, numFrames, frameRate, sharpen, edgeEnhance, 
	 * 		movie format, showImgIfGap, emailUser, emailAddress, quality, image format.
	 * Settings that ARE affected when the viewport changes: startTime/EndTime, layers, filename.
	 * 
	 * TimeStep currently sticks with whatever the user picks. If they change it in the viewport, MediaSettings.timeStep
	 * 		is not changed or affected. May need to change this.
	 ...
	 * 
	 * Settings that are stored but not functional yet: frameRate, sharpen, edgeEnhance, image format, showImgIfGap, emailUser, 
	 * 		emailAddress, startdate/enddate, filename
	 * 
	 ...
	 * 				These are all settings that will change whenever the viewport changes.
	 * 
	...
	getSettings: function (visibleCoords) {
		...
		// startTime is in unix timestamp format in seconds. date.getTime() returns milliseconds so it needs to be divided by 1000.
		...
		maxImgSize = helioviewer.tileLayers.getMaxJP2Dimensions(visibleCoords.left, visibleCoords.top, vpWidth, vpHeight);
	 ...
	 * 				and adds those layers that are visible to an array. Returns the array of layers. The filename is also built
	 * 				here from each layer's detector and measurement.
	...
	getImageInformation: function (visibleCoords, vpWidth, vpHeight) {
		...
		jQuery.each(helioviewer.tileLayers, function () {
			// "this" represents the tilelayer object
			if (this.visible) {
				var left, top, right, bottom, width, height, sizeOffset, jp2Width, jp2Height, xRange, yRange, relWidth, relHeight, layer = [];
				...
				if (this.detector.toString() === "0C2" && this.insideCircle(216, jp2Width / 2, left, top, width, height)) { 
					width = -1;
				}
				// 104 is the radius of the transparent circle in a C3 image.
				else if (this.detector.toString() === "0C3" && this.insideCircle(104, jp2Width / 2, left, top, width, height)) { 
					width = -1;
				}
		...
		jQuery.each(layers, function (i, l) {
			var distX = 0, distY = 0;
			/* 
	...
	checkLayers: function () {
		var layers = [], i = 0;
		if (!this.layers) {
			jQuery.each(this.layerNames, function (i, layer) {
			...
			var self = this, matched = false, newLayers = [], baseLayers = [], base = "";
			...
			// still the same. 
			jQuery.each(this.layerNames, function (i, l) {
	...
	getImgCoords: function (xRange, yRange, jp2Width, jp2Height, relWidth, relHeight) {
		var imgCoords = [], xStart = xRange.start, xSize = xRange.size, yStart = yRange.start, ySize = yRange.size, relXSize, relXStart, relYSize, relYStart;
		imgCoords = {
			left	: 0,
			top		: 0,
			right	: 0,
	...
	initSettingsDialog: function (self) {
        ...
        jQuery.post('dialogs/settings.php', params, callback, "json");
	},
		...
		Shadowbox.open({
			...
			options:	{
				...
				onFinish: function () {
					 ...
					 */
					jQuery("#shadowbox-form")[0].style.display = "";
					...
					jQuery('#submit-options-button').click(function () {
						...
						jQuery.extend(self, userSettings);
