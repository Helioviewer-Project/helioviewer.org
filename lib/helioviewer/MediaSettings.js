...
var MediaSettings = Class.create(
	...
	checkLayers: function () {
		var layers = [], i = 0;
		if (!this.layers) {
			this.layerNames.each(function (layer) {
			...
			var self = this, newLayers = [], baseLayers = [], index = -1, i = 0, base = "";
			...
			// still the same. 
			this.layerNames.each(function (l) {
			...
			baseLayers.each(function (bl) {
				var base = bl.split(",").slice(0, 4).join(",");
				
				// Find where this particular layer is in baseLayers. 
				index = baseLayers.indexOf(base);

				// If it exists in layers and is set to false, add it as false to newLayers
				if (self.layers.indexOf(base + " false") >= 0) {
					layers[index] = base + " false";
				}
				
				// Otherwise set it to true
				else {
					layers[index] = base + " true";
					newLayers.push(self.layerNames[index]);
				}
			});
	...
	getImgCoords: function (xRange, yRange, jp2Width, jp2Height, relWidth, relHeight) {
		var imgCoords = $A(), xStart = xRange.start, xSize = xRange.size, yStart = yRange.start, ySize = yRange.size, relXSize, relXStart, relYSize, relYStart;
		imgCoords = {
			left	: 0,
			top		: 0,
			right	: 0,
	...
	initSettingsDialog: function (self) {
		var startTime = self.startTime, endTime = new Date();
