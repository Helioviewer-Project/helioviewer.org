...
var MediaSettings = Class.create(
	...
	initialize: function (controller) {
		...
		this.getSettings(this, visibleCoords);
	 ...
	 */
	getSettings: function (self, visibleCoords) {
		var helioviewer = self.controller, vpWidth, vpHeight, maxImgSize, imgWidth, imgHeight;
		...
		self.filename  = (new Date()).getTime();
		...
		self.layerNames = self.getImageInformation(visibleCoords, vpWidth, vpHeight, self);
	 ...
	 */
	getImageInformation: function (visibleCoords, vpWidth, vpHeight, self) {
		var layers = $A(), helioviewer, layerNames = $A(), imgCoords = $A();
		...
		helioviewer.tileLayers.each(function () {
			...
			if (this.visible) {
				...
				else if (this.detector.toString() === "0C3" && left >= 637 && right <= 781 && top >= 637 && bottom <= 781) {
					width = -1;
				}
				...
				if (width > 0 && height > 0 && top < this.height && left < this.width) {
					...
					if (relWidth >= self.width - 2 || relHeight >= self.height - 2) {
						imgCoords = self.getImgCoords(self, xRange.start, xRange.size, yRange.start, yRange.size, jp2Width, jp2Height, relWidth, relHeight);
					}
	 ...
	 */	
	getImgCoords: function (self, xStart, xSize, yStart, ySize, jp2Width, jp2Height, relWidth, relHeight) {
		...
		if (xStart == 0) {
			...
			imgCoords.left 	= imgCoords.right - self.width;
		}
