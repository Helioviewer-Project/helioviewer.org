...
var Helioviewer = Class.create(
	...
	loadUserSettings: function () {
		...
		if (this.load.imageLayers) {
			var server, layer, layers = [], self = this;
            
			$A(this.load.imageLayers).each(function (layerString) {

                // Choose server to use
                if ((self.distributed === true) && ((layers.length % 2) == 0))
                    server = self.tileServer2;
                else
                    server = self.tileServer1;
                
                // Parse layer string
                layer = self.userSettings.parseLayerString(layerString);
	...
	_createScreenshotButton: function () {
		...
		this.button.click(function () {
			if (self.building) {
				...
				self.imageSelectTool.takeScreenshot(visibleCoords, self);
			}
		});
	},
	...
	getViewportInformation: function (visibleCoords) {
		var layers, hcOffset, tileLayers, viewportCoords, centerX, centerY;
		...
		hcOffset = {
			...
			y: visibleCoords.top
		}; //this.getHCOffset(visibleCoords);
		...
		tileLayers.each(function (t) {
			if (t.visible) {
				...
				if (width > 0 && height > 0 && top < t.height && left < t.width) {
					...
					// Layer names are in the format [OBS,INST,DET,MEAS,visible,opacity] to allow for variable-length names 
//					name = $A([t.observatory, t.instrument, t.detector, t.measurement]);

					layerNames.push(t + "x" + xRange.start + "," + xRange.size + "," + yRange.start + "," + yRange.size);//(name.join("_"));
