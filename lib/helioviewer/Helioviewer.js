 ...
 * 
 * Remaining classes to covert:
 *     EventLayerAccordion, EventTimline, MediaSettings, MovieBuilder, TileLayerAccordion, UserSettings
 * 
 * Syntax: jQuery (x)
 */

/*global Class, Calendar, EventLayerAccordion, EventLayerManager, EventTimeline, LayerManager, ImageSelectTool, MediaSettings, MovieBuilder, TileLayerManager, MessageConsole, Shadowbox, TileLayer, TileLayerAccordion, TimeControls, UserSettings, ZoomControls, jQuery, Viewport, document, console, window, Time */
var Helioviewer = Class.create(
	...
	initialize: function (viewportId, api, view, defaults) {
		...
		// Add initial layers
        var self = this;
		jQuery.each(this.userSettings.get('tileLayers'), function () {
			self.tileLayers.addLayer(new TileLayer(self.viewport, this));
		});
	},
	...
	_initUI: function () {
        ...
        mouseCoords = jQuery('<div id="mouse-coords" style="display: none;"></div>');
		...
		this.viewport.innerNode.append(mouseCoords);
    ...
    _checkBrowser: function () {
        ...
        //jQuery.support.nativeJSON = (typeof(JSON) !== "undefined") ? true: false;
        jQuery.support.nativeJSON = false;
        
        // Web storage (local)
        jQuery.support.localStorage = ((typeof(localStorage) !== "undefined") && (localStorage !== null)) ? true: false;
        ...
        //jQuery.support.textShadow = ((navigator.userAgent.search(/Firefox\/[1-3]\.[0-1]/) === -1) && (!jQuery.browser.msie)) ? true : false;
        jQuery.support.textShadow = false;
        

        // Add JSON support to local storage
        if (jQuery.support.nativeJSON && jQuery.support.localStorage)
			extendLocalStorage();
    ...
    _getDataSources: function () {
        ...
        jQuery.post(this.api, {action: "getDataSources"}, callback, "json");
    },
	...
	loadUserSettings: function () {
		...
		if (this.load.imageLayers) {
			var layerSettings, layers = [], self = this;
       		...
       		jQuery.each(this.load.imageLayers, function () {
	...
	_initToolTips: function () {
		...
		jQuery.each(items, function () {
			self.addToolTip(this, {yOffset: -125});
		});
