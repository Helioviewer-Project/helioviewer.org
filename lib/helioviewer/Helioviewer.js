...
var Helioviewer = Class.create({
    ...
    handlers: {
	    ...
	    observationTimeChange: function (e) {
	        //TODO: NEED TO FIRST UPDATE this.date TO REFLECT CHANGES!
	        //Debug.output("time changed!.. new value: " + e.target.value);
	        var time = e.target.value;
	        
	        //make sure time entered in correct format
	        var regex = /^\d{2}:\d{2}:\d{2}?/;
	        if (time.match(regex)) {
                this.date.setHours(parseInt(time.substring(0,2)));
                this.date.setMinutes(parseInt(time.substring(3,5)));
                this.date.setSeconds(parseInt(time.substring(6,8)));
                
                //Adjust for UTC time
                var UTCOffsetStr = this.date.getUTCOffset();
                var UTCOffset    = parseInt(UTCOffsetStr.match(/[1-9]/)[0]);
                if (UTCOffsetStr.match('-')) {
                    UTCOffset = -UTCOffset;
                }
                this.date.addHours(UTCOffset);
	            this.setDate(this.date);
	            
	            
	            //TODO: Still need to update *color* of datefield to reflect whether times match or not
