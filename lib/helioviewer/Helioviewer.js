...
//		Shadowbox.init({skipSetup: true});
	...
	_initUI: function () {
		...
		this._createSelectRegionButton();
	...
	_createSelectRegionButton: function() {
		var button = jQuery("#select-region-button");		
		self = this;
	
		button.click(function() {
			var width, height, transImg;
			width = self.viewport.dimensions.width;
			height = self.viewport.dimensions.height;
			transImg = self.viewport.domNode.appendChild(Builder.node('img', {
...
/*					var overlay = self.viewport.domNode.appendChild(Builder.node('div', {class: 'ui-overlay'}));
					oWidget = overlay.appendChild(Builder.node('div', {class: 'ui-widget-overlay'}));
					oWidget.appendChild(Builder.node('div', {
						class: 'ui-widget-shadow ui-corner-all',
						style: 'width: 302px; height: 152px; position: absolute; left: 50px; top: 30px;'
					}));
*/					
					var overlay = self.viewport.domNode.appendChild(Builder.node('div', {
						class: 'ui-widget ui-widget-content ui-corner-all',
						style: 'padding: 10px; position: absolute; z-index: 6; width: 280px; height: 130px; left: 50px; top: 30px;'
					}));
					var content = overlay.appendChild(Builder.node('p', "What would you like to do?"));
					self.viewport.domNode.removeChild(transImg);
/*					content.appendChild(Builder.node('span', {
						id: 	'build-movie',
						title: 	" Build Movie ", 
					...
					content.appendChild(Builder.node('span', {
						...
						class:	'ui-icon ui-icon-image',
						style: "cursor:pointer; display: inline-block; float: left;"
					}));
					
					mButton = jQuery("#build-movie");
					sButton = jQuery("#take-sreenshot");
					
					mButton.click(function() {
						alert("Clicked build movie!");
					});
					sButton.click(function() {
						alert("Clicked take screenshot!");
					});
*/
