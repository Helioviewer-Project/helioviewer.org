...
var Helioviewer = Class.create(
	...
	getViewportInformation: function(visibleCoords) {
		var layers, hcOffset, layerNames = $A(), layerOpacities = $A(), layerXRanges = $A(), layerYRanges = $A();
		...
		this.tileLayers.each(function (t) {
			if (t.visible) {
				var left, top, right, bottom, width, height, scaleOffset, jp2Width, xRange, yRange, name;
				...
				scaleOffset = jp2Width / t.relWidth;
				...
				// Using naturalImageScale doesn't work right.
//		        currentScale = self.viewport.controller.baseScale * Math.pow(2, self.viewport.zoomLevel - self.viewport.controller.baseZoom);
//		        scaleOffset  = t.naturalImageScale / currentScale;
	
				left 	= Math.max(left * scaleOffset, 0);
				top 	= Math.max(top  * scaleOffset, 0);
				right 	= right  * scaleOffset;
				bottom 	= bottom * scaleOffset;
				...
				// Round them off to the nearest integer, since you cannot have half-pixels as a measurement
				...
				if (width > 0 && height > 0 && top < t.height && left < t.width) {
					...
					layerNames.push(t.toString() + "x" + xRange.start + "," + xRange.size + "," + yRange.start + "," + yRange.size);
				}
			}
		});
