...
var Helioviewer = Class.create(
	...
	loadUserSettings: function () {
		...
		var obsdate = 1041379201;
		var img_scale = 10;
//		var layers = "SOHEITEIT304";
		// Load any view parameters specified via API
//		if (this.load["obs-date"]) {
			this.userSettings.set('obs-date', obsdate); //parseInt(this.load["obs-date"], 10) * 1000);
//		}
	...
	_createScreenshotButton: function() {
		...
		this.button.click(function() {
			if (self.building) 
				...
				obsDate = self.date.getTime();
				self.building = true;
				
				// Get all of the layer information
				layers = self.getViewportInformation(), layerNames, layerOpacities, layerXRanges, layerYRanges;
				...
				xhr = new Ajax.Request(self.url, {
					...
					onComplete: function(transport){
						...
						var linkId = self.messageConsole.link('', 'Click here to view screenshot');
						var url = transport.responseJSON; //self.url + '?action=playMovie&format=' + hqformat + '&url=' + transport.responseJSON;
	...
	getViewportInformation: function() {
		...
		tileLayers.each(function(t){
			if (t.visible) {
				...
				// If the viewport width or height is smaller than the standard imagesize, make it the imagesize.				
//				width  = Math.max(width, 512/sizeOffset);
//				height = Math.max(height, 512/sizeOffset);
...
//console.log(name + "-relative: " + t.relWidth + "," + t.relHeight);				
//console.log(name + "-actual: " + width + "," + height);	
