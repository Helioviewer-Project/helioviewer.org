 ...
 * Syntax: jQuery (x)
 */
/*global Class, $, Builder, Element, jQuery, ViewportHandlers, document */
var Viewport = Class.create(
	...
	initialize: function (controller, options) {
		jQuery.extend(this, this.defaultOptions);
		jQuery.extend(this, options);
		
		var center, centerBox;

		this.domNode            = jQuery(this.id);
		this.innerNode          = jQuery(this.id + '-container-inner');
		this.outerNode          = jQuery(this.id + '-container-outer');
        ...
        this.headerAndFooterHeight = jQuery(this.headerId).height() + jQuery(this.footerId).height() + 9;
        ...
        this.sandbox = jQuery('<div id="sandbox" style="position: absolute; width: 0; height: 0; left: ' + center.x + 'px; top: ' + center.y + 'px;"></div>');
        this.domNode.append(this.sandbox);
		
		// Create a master container to make it easy to manipulate all layers at once
        this.movingContainer = jQuery('<div id="moving-container" style="left: 0; top: 0"></div>').appendTo(this.sandbox);
	...
	resize: function () {
		...
		h = Math.max(this.minHeight, jQuery(document).height() - padHeight);
