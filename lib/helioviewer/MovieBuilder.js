...
var MovieBuilder = Class.create(
	...
	initialize: function (controller) {
		...
		this.button.click(function () {			
			...
			if (!self.building) {
				...
				// they will have to pick 3. Their choice is saved in mediaSettings.
	...
	checkMovieLayers: function (self, visibleCoords) {
		...
		if (layers.length <= 3 && layers.length >= 1) {
			self.buildMovie(self, visibleCoords, layers);
		}	
			...
			Shadowbox.open({
				...
				options:	{
					enableKeys: false,
					onFinish: function () {
						// Fill the table with each layer name.
						table = $('layers-table');
						tableValues = "";
						...
						jQuery('#ok-button').click(function () {
							...
							// "c" represents the place in the array, not the object itself. I'm not sure why.
							checkboxes.each(function (c) {
							    ...
							    if (checkboxes[c].firstChild.checked) {
									finalLayers.push(checkboxes[c].firstChild.value);
								}
							});

							if (finalLayers.length <= 3 && finalLayers.length >= 1) {
								self.buildMovie(self, visibleCoords, finalLayers);
								Shadowbox.close();
							}
	...
	buildMovie: function (self, visibleCoords, layers) {
