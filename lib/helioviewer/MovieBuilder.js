...
var MovieBuilder = Class.create(
	...
	checkMovieLayers: function (visibleCoords) {
		...
		layers = this.mediaSettings.layers;
	...
	buildMovie: function (layers) {
		var helioviewer = this.controller, self, hqFormat, mediaSettings, timeout, imgHeight, imgWidth, options, filename, xhr;
		...
		xhr = new Ajax.Request(self.url, {
			...
			onComplete: function (transport) {
				...
				if (transport.responseJSON === null) {
					...
					helioviewer.messageConsole.info("<div id='watch-" + filename + "' style='cursor: pointer'>Click here to watch it</div>" +
						"-or-<br />" + 
