...
var MovieBuilder = Class.create(
	...
	buildMovie: function (self, visibleCoords) {
		var helioviewer = self.controller, hqFormat, mediaSettings, timeout, imgHeight, imgWidth, options, filename, xhr, dateString, layers;
				...
				options:	{
					...
					onFinish: function () {
						...
						layers.each(function (l) {
							...
							name = rawName.join(" ");
							table.innerHTML += '<tr>' +
									'<td class="layers-checkbox"><input type=checkbox name="layers" checked=true value="' + name + '"/></td>' +
