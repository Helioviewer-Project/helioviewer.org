 ...
 * @author Jaclyn Beck
 */
/*global Class, jQuery, $, $A, Ajax, Builder, Shadowbox, setTimeout */
var MovieBuilder = Class.create(
	...
	buildMovie: function (self, visibleCoords) {
		var helioviewer = self.controller, zoomLevel, startTime, timeStep, hqformat, mediaSettings, timeout, layers = $A(), layerNames = $A(), imgHeight, imgWidth, vpWidth, vpHeight, sbWidth, sbHeight, maxImgSize, hcOffset, url, xhr, dateString, showImgIfGap, emailUser, emailAddress, vpDimensions;
		...
		startTime = mediaSettings.startTime;
		...
		layerNames 	= mediaSettings.layers; //layers.layerNames;
		hcOffset 	= mediaSettings.hcOffset; //layers.hcOffset;
		hqformat  	= mediaSettings.hqFormat;
		hcOffset	= mediaSettings.hcOffset;

		showImgIfGap = mediaSettings.showImgIfGap;
		emailUser	 = mediaSettings.emailUser;
		emailAddress = mediaSettings.emailAddress;
