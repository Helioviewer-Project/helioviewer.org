...
var MovieBuilder = Class.create(UIElement, {
	...
	initialize: function (controller) {
		...
		this.button.click(function () {			
			...
			if(!self.building) {
				...
				movieSettings = self.getUserInformation(self, visibleCoords);
				
				//self.buildMovie(self, movieSettings);
			}
		});
	},
	 ...
	 */
	getUserInformation: function (self, visibleCoords) {
	 ...
	 */
	printContent: function (contents) {
		Shadowbox.open({
			player:	"html",
			width: 	450,
			height: 450,
				...
				onFinish: function() {
					...
					jQuery('#submit-options-button').click(function() {
						//movieSettings = $A({
							...
							vpWidth 	= jQuery('#width').val();
							vpHeight 	= jQuery('#height').val();
