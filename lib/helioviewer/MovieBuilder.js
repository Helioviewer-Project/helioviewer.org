...
var MovieBuilder = Class.create(
	...
	checkMovieLayers: function (visibleCoords) {
		var finalLayers = $A(), self, layers, table, info, rawName, name, tableValues, checkboxes;
	...
	buildMovie: function (layers) {
		var helioviewer = this.controller, self, mediaSettings, timeout, imgHeight, imgWidth, options, xhr;
				...
				if (transport.responseJSON === null) {
					...
					options = {
						...
						open:	function () {
							...
							hqfile = (transport.responseJSON).slice(0, -3) + mediaSettings.hqFormat;
							
							download = jQuery('#download-' + mediaSettings.filename);
							watch	 = jQuery('#watch-' + mediaSettings.filename);
					...
					helioviewer.messageConsole.info("<div id='watch-" + mediaSettings.filename + "' style='cursor: pointer'>Click here to watch it</div>" +
						"-or-<br />" + 
						"<div id='download-" + mediaSettings.filename + "' style='cursor: pointer'>Click here to download a high-quality version.</div>", options);
				} 
			}
		}); 
	}, 
