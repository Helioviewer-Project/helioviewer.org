...
var TileLayerManager = LayerManager.extend(
   ...
   init: function(controller) {
	...
	getMaxJP2Dimensions: function (left, top, width, height) {
		...
		$.each(this._layers, function () {
			if (this.visible) {
				if (this.detector.toString === "C2" && this.insideCircle(216, this.jp2Width / 2, left, top, width, height)) {
					; // Do nothing
				}
				if (this.detector.toString === "C3" && this.insideCircle(104, this.jp2Width / 2, left, top, width, height)) {
					; // Do nothing
				}
