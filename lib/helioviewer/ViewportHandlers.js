...
var ViewportHandlers = Class.create(
	...
	mouseDown: function (event) {
        ...
        vp = jQuery("#helioviewer-viewport");
        sb = jQuery("#sandbox");
	...
	mouseUp: function (event) {
		jQuery("#helioviewer-viewport").css("cursor", "pointer");
	...
	mouseWheel: function (e, delta) {
		...
		this.viewport.controller.zoomControls.zoomButtonClicked(dir);
	},
	...
	getRelativeCoords: function (screenx, screeny) {
		...
		offset = jQuery('#helioviewer-viewport-container-inner').position();
    ...
    updateShadows: function () {
        ...
        if (!jQuery.support.textShadow)
            return;
        
        var viewportOffset, sunCenterOffset, coords, viewportCenter, offsetX, offsetY;
        
        viewportOffset  = jQuery("#helioviewer-viewport").offset();
        sunCenterOffset = jQuery("#moving-container").offset();
	...
	toggleMouseCoords: function () {
		...
		if (this.viewport.controller.userSettings.get('warnMouseCoords') === false) {
			this.viewport.controller.messageConsole.log("<b>Note:</b> Mouse-coordinates should not be used for science operations!");
			this.viewport.controller.userSettings.set('warnMouseCoords', true);
		}
		
		// Cartesian & Polar coords
		if (this.viewport.mouseCoords !== "disabled") {
			mouseCoordsX = jQuery('#mouse-coords-x');
			mouseCoordsY = jQuery('#mouse-coords-y');
			...
			if (this.viewport.mouseCoords === "polar")
                jQuery('#moving-container').unbind('mousemove', updateMouseCoords);
			...
			updateMouseCoords = function (e) {
				...
				// Polar coords
					...
					mouseCoordsX.html(((polar.r / self.rSunArcSeconds) + "").substring(0, 5) + " R<span style='vertical-align: sub; font-size:10px;'>&#9737;</span>");
					mouseCoordsY.html(Math.round(polar.theta) + " &#176;");
				}
			};
            jQuery('#moving-container').bind('mousemove', updateMouseCoords);	
    ...
    computeMouseCoords: function(screenX, screenY) {
		...
		negSV = jQuery("#sandbox").position();
		SV = {
			x: -negSV.left,
			y: -negSV.top
		};
		SM = jQuery('#moving-container').position();				
		MX = {
			x: VX.x + (SV.x - SM.left),
			y: VX.y + (SV.y - SM.top)
		};
    ...
    _initEvents: function () {
        ...
        doc.mousemove(function (e) { self.mouseMove(e); });
        doc.mouseup(function (e) { self.mouseUp(e); });
        vp.mousedown(function (e) { self.mouseDown(e); });

		// Double-clicks
        vp.dblclick(function (e) { self.doubleClick(e); });

		// Mouse-wheel
        vp.mousewheel(function (e, delta) { self.mouseWheel(e, delta); });
    }
});
