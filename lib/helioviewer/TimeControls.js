 ...
 */
/*global TimeControls, Class, Event, Element, $, $A */
var TimeControls = Class.create(UIElement,
     ...
     */
    initialize : function (controller, increment, dateInput, timeInput, incrementSelect, backBtn, forwardBtn) {
        this.controller = controller;
        
        // UI components
        this._dateInput  = jQuery(dateInput);
        this._timeInput  = jQuery(timeInput);
        this._backBtn    = jQuery(backBtn);
        this._forwardBtn = jQuery(forwardBtn);
        this._incrementSelect = jQuery(incrementSelect);
    ...
    _updateInputFields: function () {
        ...
        this._dateInput.val(date.toYmdUTCString());
        this._timeInput.val(date.toHmUTCString());
    },
    ...
    */
    ...
    _addTimeIncrements: function () {
        ...
        jQuery(timeSteps).each(function (i, timestep){
            opt = jQuery("<option value='" + timestep.numSecs + "'>" + timestep.txt + "</option>");
            select.append(opt);
        });
		
		// Select default timestep
		$('timestep-select').select('option[value=' + this._timeIncrement + ']')[0].writeAttribute('selected', 'selected');
        
        //TODO: Select initial value using jQuery!
    ...
    */
    ...
    _initDatePicker: function() {
        ...
        // Datepicker icon
        btn = jQuery('.ui-datepicker-trigger');
