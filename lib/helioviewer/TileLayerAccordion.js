...
var TileLayerAccordion = Layer.extend(
	...
	addLayer: function (layer) {
		...
		var visibilityBtn, removeBtn, hidden, head, body, slider, ids, selected, self = this;
        ...
        ids = ["#observatory-select-" + layer.id, "#instrument-select-" + layer.id, "#detector-select-" + layer.id, "#measurement-select-" + layer.id];
        selected  = [layer.observatory, layer.instrument, layer.detector, layer.measurement];
        this.selectMenus = new TreeSelect(ids, this.controller.dataSources, this._onSelectChange, selected);
       ...
       // params = {
		...
		//Ajax Request
		//$.post(this.queryURL, params, callback, "json");
	},
	...
	_setupEventHandlers: function (layer) {
		...
		$.each($('#' + layer.id + ' > div > select'), function (i, item) {
    ...
    _onSelectChange: function (e, layer) {
        console.log(layer);
    },
    ...
    updateTimeStamp: function (layer) {
    	...
    	domNode = $("#" + layer.id).find('.timestamp');
