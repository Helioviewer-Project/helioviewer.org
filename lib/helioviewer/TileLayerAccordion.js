...
var TileLayerAccordion = Class.create(Layer, {
	...
	addLayer: function (layer) {
		...
		var processResponse = function(transport){
			...
			jQuery("#opacity-slider-" + layer.id).slider({
				startValue: layer.opacity * 100,
				change: function(e, ui) {
					var val = ui.value;
					//Debug.output("Updating opacity to " + val);
	...
	_buildEntryBody: function (layer) {
		...
		var opacitySlide = "<div class='opacity-slider-track ui-slider' id='opacity-slider-track-" + id + "' style='margin:10px;'>";
		opacitySlide += "<div class='opacity-slider-handle ui-slider-handle' id='opacity-slider-handle-" + id + "'></div>"
		opacitySlide += "</div>";
