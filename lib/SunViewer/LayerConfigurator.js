...
LayerConfigurator.prototype = {
	...
	createElements: function (row) {
		...
		if (this.layerProvider.type === 'TileLayerProvider') {
			...
			//There really should be an easier way to tell what instrument you are looking at...
			if (this.layerProvider.sunImage.instrument === "EIT") {
			    row.appendChild(this.createWavelengthControl());
			}

    		row.appendChild(this.createOpacityControl());
		} else if (this.layerProvider.type === 'OverlayLayerProvider') {
			desc = new Element('td', {colSpan: '3'}).update('Overlay Layer');
			row.appendChild(desc);
		} else if (this.layerProvider.type === 'MarkerLayerProvider') {
			numberOfMarkerLayers++;
            desc = new Element('td', {colSpan: '3'}).update('Marker Layer');
			row.appendChild(desc);
		}
	 ...
	 */
	 createWavelengthControl: function () {
	    this.wavelengthHtmlElement = new Element('select', {id: 'wavelengthSelect'});
	    
	    this.wavelengthHtmlElement.length = 0;
        ...
        for (var i = 0; i < wavelengths.length; i++) {
            ...
            this.wavelengthHtmlElement.options[this.wavelengthHtmlElement.options.length] = opt;
        }
        
        // Set-up event handler to deal with an wavelength change
        Event.observe(this.wavelengthHtmlElement, 'change', this.onWavelengthChange.bind(this));
	    
        return new Element('td', {'class': 'layerConfigurator'}).update(this.wavelengthHtmlElement);
    },
     ...
     */ 
    onWavelengthChange: function () {
        var wl = this.wavelengthHtmlElement.options[this.wavelengthHtmlElement.selectedIndex].value;
        var id = this.layerProvider.id;
        
        document.wavelengthChange.fire(id, wl);
    },
