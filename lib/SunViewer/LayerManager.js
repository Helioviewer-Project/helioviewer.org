...
LayerManager.prototype = Object.extend(new SunViewerWidget(), {
	...
	initialize: function(tileContainer, elementId, options) {
		...
		//[TESTING] if next/previous image is available for a given filter, update image in layerconfig->sbdm->selectedImage...
		document.tmpHandleImgChange.subscribe(this.onTmpHandleImgChange, this, true);
	...
	//TMP
	onTmpHandleImgChange: function (type, args) {
        var num = args[0];
        var img = args[1];
        this.layerConfigurators[num].imageSelect.selectedImage = img;
        this.layerConfigurators[num].imageSelect.notifyListeners('ImageChange', img);
	},
	...
	addEffects: function() {
		...
		var growingFinished = function() {
			active = false; this.expanded = true;
			this.toggleButton.innerHTML = '&nbsp;-&nbsp;';
		};
		var shrinkingFinished = function() {
			active = false; this.expanded = false;
			this.toggleButton.innerHTML = '&nbsp;+&nbsp;';
		...
		var mouseClickHandler = function(e) {
			...
			if (this.expanded) {
				...
				//Resume dragging
				//Event.observe($('ui'), 'mousedown', this.viewport.surface.mousePressedHandler.bindAsEventListener(this.viewport.surface));
				...
				//Event.stopObserving ($('ui'), 'mousedown');
	...
	createElements: function() {
		...
		var table = new Element('table', {cellSpacing: '0', width:'500'});
	...
	addLayer: function(layerProvider) {
		...
		////[[TESTING]] FIRE AN EVENT TO UPDATE DATA NAVIGATOR TO HANDLE NEW LAYER (what does the filter constructor actually NEED?)
		//instrument: layerProvider.sunImage.detector
		
		//document.newLayer.fire(layerProvider.sunImage.detector);
		document.newLayer.fire("EIT");
