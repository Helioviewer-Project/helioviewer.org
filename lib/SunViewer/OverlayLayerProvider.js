...
OverlayLayerProvider.prototype = Object.extend(new TileLayerProvider(), {
	...
	createTileLayer: function (tile) {
		...
		if (this.sunImage.instrument === "EIT")	{
			...
			if (zoomLevel >= this.oneTileSunZoomLevel)	{
				tileIndexOffset = 0;
			}
			else if (zoomLevel === this.oneTileSunZoomLevel -1) {
				tileIndexOffset = -0.5;
			}
			else if (zoomLevel < this.oneTileSunZoomLevel -1) {
				tileIndexOffset = 0.5;
				for (Counter = this.oneTileSunZoomLevel -1; Counter >= 0; Counter--) {
		...
		var tileOverlays = this.overlays.getOverlays(xIndex, yIndex, (this.oneTileSunZoomLevel - zoomLevel));
		...
		tileOverlays.each(function (overlay) {
			...
			var x = (Math.pow(2, (this.oneTileSunZoomLevel - zoomLevel)) * overlay.position.x - xIndex) * self.tileContainer.tileSize + tileOffset;
			var y = (Math.pow(2, (this.oneTileSunZoomLevel - zoomLevel)) * overlay.position.y - yIndex) * self.tileContainer.tileSize + tileOffset;
	...
	onNewOverlay: function (type, args) {
		...
		if (!overlay.sunImage.date || overlay.sunImage === this.sunImage) {
			...
			if (this.layerCache.contains(zoomLevel, xIndex, yIndex)) {
				...
				var x = ((1 << (this.oneTileSunZoomLevel - zoomLevel)) * overlay.position.x - 0.5) * this.tileContainer.tileSize + tileOffset;
				var y = ((1 << (this.oneTileSunZoomLevel - zoomLevel)) * overlay.position.y - 0.5) * this.tileContainer.tileSize + tileOffset;
