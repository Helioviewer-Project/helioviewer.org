...
OverlayLayerProvider.prototype = Object.extend(new TileLayerProvider(), {
	...
	onNewOverlay: function(overlay) {
		if (!overlay.sunImage.date || overlay.sunImage == this.sunImage) {
		    ...
		    var zoomOffset = this.zoomOffset;
		    var zoomLevel = this.tileContainer.viewport.zoomLevel;
		    var tileIndexOffset = (Math.pow(2, -zoomOffset) - 1) * Math.pow(2, this.tileContainer.viewport.zoomLevel + zoomOffset - 1);
			...
			if (this.layerCache.contains(zoomLevel, xIndex, yIndex)) {
				...
				var x = ((1 << zoomLevel) * overlay.position.x - 0.5) * this.tileContainer.tileSize + tileOffset;
				var y = ((1 << zoomLevel) * overlay.position.y - 0.5) * this.tileContainer.tileSize + tileOffset;
