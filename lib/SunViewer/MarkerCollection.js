 ...
 */
 /*global Class, $A, LatLon, Marker, AjaxRequestWrapper */
...
MarkerCollection.prototype = Object.extend($A([]), {
	...
	initialize: function (overlays)  {
	    this.className = "MarkerCollection";
	...
	loadMarkers: function (sunImage) {
	    //Debug.output("MarkerCollection:loadMarkers()...");
		...
		var trash = new AjaxRequestWrapper.getCached(fileUrl, this.createFromSrsTxt.bind(this), fileUrl, sunImage);
	},
		...
		for (var lnum = 0; lnum < lines.length; lnum++) {
			...
			if (first2chars === 'I.') {
				...
				while (isFinite(words[0])) {
					...
					var positionDeg = {
						latitude: parseFloat(positionDegStr.substr(1, 2), 10) * (words[1].substr(0, 1) === 'S' ? 1 : -1),
						longitude: parseFloat(positionDegStr.substr(4, 2), 10) * (words[1].substr(3, 1) === 'W' ? 1 : -1) 
					};
					...
					var info = 	'<b>Region with Sunspots</b><br/>' +
								...
								'NN: ' + words[6] + '<br/>' +
								'Mag Type: ' + words[7] + '<br/>' + 
								'<br><a href="' + fileUrl + '" target="_new" class="infoBubbleLink">(SRS text file)</a>';

					// Create the marker
					//Debug.output("New Marker! " + words[0]);
