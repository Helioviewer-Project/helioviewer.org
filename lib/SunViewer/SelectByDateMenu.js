...
SelectByDateMenu.prototype =  Object.extend(new SunViewerWidget(), {
	...
	createElements: function() {
		...
		this.dayHtmlElement = new Element('select', {id: this.domNode.id + '.DaySel'});
      	this.imageHtmlElement = new Element('select', {id: this.domNode.id + '.ImageSel'});
	...
	populateElements: function() {
		...
		var url = this.Database + 'ReturnInstruments.php' + '?dummy=' + new Date().getTime();
	...
	populateInstruments: function(txt) {
		...
		var url = this.Database + 'ReturnYears.php?Instrument=' + this.map.instrument + '&dummy=' + new Date().getTime();
		new AjaxRequestWrapper.getCached(url, callback);
	},
	...
	populateYears: function(txt) {
		...
		var url = this.Database + 'ReturnMonths.php?Instrument=' + this.map.instrument + '&Year=' + this.date.year + '&dummy=' + new Date().getTime();
		new AjaxRequestWrapper.getCached(url, callback);
	},
	...
	populateMonths: function(txt) {
		...
		var url = this.Database + 'ReturnDays.php?Instrument=' + this.map.instrument + '&Year=' + this.date.year + '&Month=' + this.date.month + '&dummy=' + new Date().getTime();
		new AjaxRequestWrapper.getCached(url, callback);
	},
	...
	populateDays: function(txt) {
		...
		var url = this.Database + 'ReturnImages.php?Instrument=' + this.map.instrument + '&Year=' + this.date.year + '&Month=' + this.date.month + '&Day=' + this.date.day  + '&dummy=' + new Date().getTime();
		new AjaxRequestWrapper.getCached(url, callback);
	...
	populateImages: function(txt) {
		...
		for (var Counter = 0; Counter < images.length; Counter++)
		{
			...
			var sunImg = new SunImage(
			{
				...
				wavelength: "",
				resolution: "",
				tileDir: "/hvdb" + "/" + this.date.year + "/" + this.date.month + "/" + StringDay + "/" + StringHour  + "/" + observatory + "/" + instrument + "/" + detector + "/" + measurement + "/" + this.date.year + "_" + this.date.month + "_" + StringDay + "_" + StringHour + StringMinute + StringSecond + "_" + observatory + "_" + instrument + "_" + detector + "_" + measurement
			});
	...
	handleInstrumentChange: function() {
	    console.log('SelectByDateMenu::handleInstrumentChange');
	...
	appendElements: function() {
		console.log('SelectByDateMenu::appendElements called!');
