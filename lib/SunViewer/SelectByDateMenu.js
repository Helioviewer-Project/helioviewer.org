...
SelectByDateMenu.prototype =  Object.extend(new SunViewerWidget(), {
	...
	initialize: function(elementId, options) {
		//[[TESTING]]
		//this.FilterCollection = new Filter();
		
	    this.className = "SelectByDateMenu";
		this.date = { year: 0, month: 0, day: 0, image: '', hour: 0, min: 0, sec: 0 };
	...
	observeElements: function() {
		...
		Event.observe(this.dayHtmlElement, 'change', function() {document.dayChange.fire(parseInt(this.value));});
		document.dayChange.subscribe(this.onDayChange, this, true);
		
		/*
		 * NOTE: Currently the functionality for loading new images (e.g. handleImageChange) is tied
		 * to the SelectByDateMenu. Ideally, this functionallity should be separated and the SelectByDate
		 * menu should only fire events which will then be caught by a functionality engine.
		 */
		 //[TESTING] Moving functionality to DataNavigator
	...
	populateDays: function(txt) {
		...
		//Set global variables to keep track of current time.. Eventually it would be better to keep track
		//of time at a different location and simply fire events when the time is altered.
        //GLOBAL_TIME_YEAR  = parseInt(this.date.year[0]);
        //GLOBAL_TIME_MONTH = parseInt(this.date.month[0]);
        //GLOBAL_TIME_DAY   = parseInt(this.date.day[0]);
	...
	populateImages: function(txt) {
		...
		//[[TESTING]]
		//this.FilterCollection.Retreive(null, this.map.instrument, null, null, this.date.year, this.date.month, this.date.day);
		
		//Debug.output("sbdm... this.map.inst, this.date.*");
		//console.dir(this);
	...
	timePrevious: function() {
	    //[[TESTING]]
	    if (this.FilterCollection.Previous())
	    {
	    	this.handleImageChange();
	    }
	},
	 ...
	 */
	 onDayChange: function (type, args) {
	...
	handleImageChange: function() {
		//[[TESTING]]
		//Debug.output("handleImageChange...")
		//console.dir(this);
		//this.selectedImage = this.FilterCollection.CurrentSunImage();
		//[[TESTING]]
