...
LayerManagerUI.prototype = {
    ...
    createMenu: function (){
        ...
        this.toggleBtn =   Builder.node('a', {href: '#', style: 'float: left; color: black; text-decoration:none; font-weight: bold; '}, 'Layers');
        ...
        tr.appendChild(Builder.node('th', 'Timestamp'));
    ...
    createMenuNEW: function () {        
        ...
        this.accordion = new accordion('layerManager-Container');
    ...
    addMenuEntryNEW: function (layerProvider) {
        ...
        switch (layerProvider.type) {
            case 'TileLayerProvider':
                // Create header
                var header =  Builder.node('h2', {className: 'accordion_toggle'}, layerProvider.sunImage.instrument);
                var content = Builder.node('div',{className: 'accordion_content'});
                content.appendChild(Builder.node('p', 'wavelength'));
                content.appendChild(Builder.node('p', 'opacity'));
                
                this.menuContainer.insert(header);
                this.menuContainer.insert(content);
                
                break;
            case 'MarkerLayerProvider':
                var header =  Builder.node('h2', {className: 'accordion_toggle'}, "Markers");
                var content = Builder.node('div',{className: 'accordion_content'}, "Option to hide/show markers layers might be better" +
                        "off separated from the rest of the layer manager...");
                this.menuContainer.insert(header);
                this.menuContainer.insert(content);
    ...
    addMenuEntry: function (layerProvider) {
        ...
        switch (layerProvider.type) {
            case 'TileLayerProvider':
                ...
                tr.appendChild(Builder.node('td', '-timestamp-'));
                ...
                // Add timestamp
                //this.menuTableTbody.appendChild(Builder.node('tr', [Builder.node('td', 'timestamp')]));
    ...
    */
    ...
    onLayerAdded: function (type, args) {
        ...
        this.addMenuEntryNEW(layerProvider);        
    },
