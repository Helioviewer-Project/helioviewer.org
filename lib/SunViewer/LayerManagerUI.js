...
LayerManagerUI.prototype = {
    ...
    */
    createMenuRowHeader: function (layerProvider) {
        ...
        // Instrument
        var instrument = Builder.node('span', {style: 'float: left'}, layerProvider.sunImage.instrument);
    ...
    createMenuRowContent: function (layerProvider) {
    	return Builder.node('div', this.createInstrumentControl(layerProvider));
    },
    ...
    onInstrumentChange: function (selfRef, id) {
        ...
        var menuItem = selfRef.layerSettings[id];
        
        //Non-pretty solution... TODO: Create a function to handle parameter changes..
        /**
        if (this.value === "LAS") {
            menuItem.select('.wavelengthSelect').first().hide();
        }
        else {
            menuItem.select('.wavelengthSelect').first().show();
        }**/
