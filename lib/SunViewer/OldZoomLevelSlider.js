...
ZoomLevelSlider.prototype =  Object.extend(new SunViewerWidget(), {
	...
	initialize: function(elementId, options) {
		...
		//console.dir(options)
		//console.log('options.maxZoomLevel = ' + options.maxZoomLevel)
	...
	linkToViewports: function() {
		...
		$A(arguments).flatten().each(function(viewport) {
			...
			//document.zoomLevelChange.subscribe(this.onZoom, viewport);
					Debug.output('viewport.zoomLevel: ' + viewport.zoomLevel + ", self.zoomLevel: " + self.zoomLevel);
			...
			//document.zoomLevelChange.subscribe(self.onZoom, self);
		});
		

		Debug.output('src1: ' + src1 + ', src2: ' + src2);
	...
	zoomControlClick: function(e, zoom, absolute) {
	    //Make sure zoom does not exceed min/max allowed
		...
		Debug.output("minZoomLevel, maxZoomLevel, newZoomLevel, zoomLevel: " + this.minZoomLevel + ", " + this.maxZoomLevel + ", " + newZoomLevel + ", " + this.zoomLevel);
	...
	setZoomLevel: function(level, source) {
		//Debug.output("[ZoomLevelSlider.setZoomLevel], level = " + level);
	...
	setMaxZoomLevel: function(level, source) {
		Debug.output("setmaxzoomlevel(" + level + ");");
