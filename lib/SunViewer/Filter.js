...
Filter.prototype = {
	...
	iterateNext: function () {
        ...
        if (this.iterator >= this.collection.length -1) {
		    var lastDate = this.currentDate().add(this.timeIncrement);
	...
	iteratePrevious: function() {
        ...
        if (this.iterator == 0) {
            ...
            var firstDate = this.currentDate().add(neg);
            
            Debug.output("Fetching new dataaset starting from: " + firstDate.toString());
            Debug.output("Iterator: " + this.iterator + ", collection.length: " + this.collection.length);
	...
	setIteratorToNearestDate: function (date) {
	    ...
	    var closestMatch =     Math.abs(curDate.getTime() - date.getTime());
	    ...
	    for (var i = 0; i < this.collection.length; i++) {
	        ...
	        if ( Math.abs(curDate.getTime() - date.getTime()) < closestMatch) {
	            newIteratorValue = i;
	        }
