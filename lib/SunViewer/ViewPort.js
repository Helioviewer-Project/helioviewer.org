...
*/
		...
		Event.observe(ui, 'mousedown', this.surface.mousePressedHandler.bindAsEventListener(this.surface));
		
		// Center control
		var controls = this.domNode.select('.centerControl');
	...
	center: function() {
		...
		document.fire('viewport:move', { x: MoveX, y: MoveY });
	...
	move: function(motion, source) {
		...
		if (motion.stop) {
			...
			Debug.output('Stopped moving, position.x: ' + this.position.x + ' position.y: ' + this.position.y);
		}
	},
