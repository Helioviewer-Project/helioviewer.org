...
TileLayerProvider.prototype = Object.extend(new SunViewerWidget(), {
	...
	assembleFileName: function(xIndex, yIndex, zoomLevel) {
		...
		if (xIndex == 0 && yIndex == 0 && this.sunImage.instrument == "EIT")
			{
			...
			if (xIndex == 0 && yIndex == 0 && this.sunImage.instrument == "LAS")
			{
				return this.sunImage.tileDir
				+ "/" + this.sunImage.date.year
				+ "/" + this.sunImage.date.month
			...
			if (zoomLevel >= 15)
				...
				else if (zoomLevel < 14)
				{
					...
					for (Counter = 13; Counter >= 0; Counter--)
					{
						...
						if (Counter == zoomLevel)
						{
							//alert(tileIndexOffset + " " + zoomLevel);
							break;
						}
					}
	...
	addTileLayerCached: function(tile) {
		...
		if (this.layerCache.contains(zoomLevel, xIndex, yIndex)) {
			...
			// add it to the cache
			//Debug.output("Adding tile layer to cache....layer: " + layer + ", zoomLevel: " + zoomLevel + ", xIndex: " + xIndex + ", yIndex: " + yIndex);
	...
	setZoomOffset: function(zoomOffset) {
		//Debug.output("TileLayerProvider:270 -> setzoomoffset layers. zoomOffset = " + zoomOffset);
...
*/
	...
	onZoomOffsetChange: function(type, args) {
		...
		this.setZoomOffset(parseInt(zoomOffset));
	}
});
