 ...
 */
 /*global document, loading, Class, $, $A, $H, Element, Event, Effect, Debug, TileLayerProvider, DomNodeCache, OverlayCollection, numberOfTileLayers, numberOfMarkerLayers, AjaxRequestWrapper, Option */
var TileLayerProvider = Class.create();

TileLayerProvider.prototype = Object.extend({
	defaultOptions: $H({
		prefix: '',
		extension: 'jpg',
	...
	assembleFileName: function (xIndex, yIndex, zoomLevel) {
	    ...
	    if (this.sunImage.date.year.length === 1) {
			this.sunImage.date.year = "0" + this.sunImage.date.year;
		}
		if (this.sunImage.date.month.length === 1) {
			this.sunImage.date.month = "0" + this.sunImage.date.month;
		}
		if (this.sunImage.date.day.length === 1)	{
			this.sunImage.date.day = "0" + this.sunImage.date.day;
		}
		...
		var StringZoomLevel = zoomLevel.toString();
		...
		if (xIndex === 0 && yIndex === 0)	{
			...
			if (this.sunImage.instrument === "EIT") {
                if (zoomLevel === 10 || zoomLevel === 11 || zoomLevel === 12) {
				...
				if (zoomLevel === 12 || zoomLevel === 13 || zoomLevel === 14 || zoomLevel === 15) {
					OffsetString = "n011.229-p011.229";
				}
				else if (zoomLevel === 16) {
					OffsetString = "n022.459-p022.459";
	...
	createTileLayer: function (tile) {
		...
		//var tileIndexOffset = (Math.pow(2, 0) - 1) * Math.pow(2, zoomLevel - 1);
		...
		if (xIndex >= 0
		 ...
		 && yIndex < nxnTiles) {
			...
			var onerror = function () {
				...
				Debug.output("Ajax Error! TileLayerProvider.js:358");
	...
	addTileLayerCached: function (tile) {
	    ...
	    if (this.layerCache.contains(zoomLevel, xIndex, yIndex)) {
            //Debug.output("TLP:addTileLayerCached... zoom: " + zoomLevel + ", xIndex: " + xIndex + ", yIndex: " + yIndex + "(CACHED)");
			// use cached version
			layer = this.layerCache.get(zoomLevel, xIndex, yIndex);
        }
        else {
			// create new layer
            //Debug.output("TLP:addTileLayerCached... zoom: " + zoomLevel + ", xIndex: " + xIndex + ", yIndex: " + yIndex + "(NOT cached)");
	...
	getFullSize: function () {
		return this.tileContainer.tileSize * Math.pow(2, this.zoomLevel);
	},
	...
	/**
	 * @function  setZoomLevel		Sets the zoom level for this layer provider.
	...
	removeFromTileContainer: function () {
        ...
        document.tileLayerAdded.unsubscribe(this.onAddTileLayer, this);
	 ...
	 */
	onAddTileLayer: function (type, args) {
	    var tile = args[0];
	...
	onImageChange: function (type, args) {
        ...
        //Check to make sure id's match. Marker layers will watch most recent layer added
        if (this.id === id) {
