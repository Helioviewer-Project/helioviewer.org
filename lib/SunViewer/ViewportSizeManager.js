...
ViewportSizeManager.prototype = {
    ...
    initialize: function (availableSizes) {
        ...
        $A(this.sizes).each((function (size) {
            $(size[0]).observe('click', this.changeSize.curry(size)); 
        }).bindAsEventListener(this));
         
        //TODO: make dynamic
        document.viewportSizeChange.subscribe(function (type, args) {
            if (args[0] !== "viewport-size-small") {
                $("viewport-size-small").setStyle({'color': '#858585'});
            }         
        });
        document.viewportSizeChange.subscribe(function (type, args) {
            if (args[0] !== "viewport-size-med") {
                $("viewport-size-med").setStyle({'color': '#858585'});
            }         
        });
        document.viewportSizeChange.subscribe(function (type, args) {
            if (args[0] !== "viewport-size-large") {
                $("viewport-size-large").setStyle({'color': '#858585'});
            }         
        });
    },
    ...
    changeSize: function (params) {
        ...
        //Note: it may be easier to create classes for the width and height
        ...
        //Everything else
        $$("#header", "#footer").each(function (s) {
            s.setStyle({width: width - 40 + "px"});
        });
    }
};