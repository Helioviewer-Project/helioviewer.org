...
/**
 * @classDescription Tile class. Represents one tile in the tile container.
 */
...
Tile.prototype = {
	...
	initialize: function(tileContainer, options) {
		...
		tile.className = 'tile';
		tile.setStyle({ width: this.tileContainer.tileSize + 'px', height: this.tileContainer.tileSize + 'px' });
	...
	position: function(domNode) {
		Debug.output("position tiles.js");
...
/**
 * @classDescription The TileContainer class manages the placing of the image tiles.
 */
var TileContainer = Class.create();

TileContainer.prototype = Object.extend(new SunViewerWidget(), {
	// Default options
	defaultOptions: $H({
		tileSize: 256,
		cols: 0,
	...
	initialize: function(viewport, options) {
		...
		this.tileLayerProviders = Object.extend($A([]), {
			add: function(tileLayer) {
				...
				this.push(tileLayer);
				container.notifyListeners('LayerAdded', tileLayer);
			},
			...
			remove: function(tileLayer) {
				...
				container.notifyListeners('LayerRemoved', tileLayer);
			},
			
			topZindex: function() {
				Debug.output("topzindex tiles");
...
//Debug.output(tileLayer2.type, tileLayer2.zIndex);
		...
		this.observe(viewport, 'Zoom');
		this.observe(viewport, 'Move');
		this.observe(viewport, 'ImageChange');
		...
		this.domNode = viewport.domNode.getElementsByClassName('well')[0].appendChild(div);

		// TODO: Create Tile Collection class
		this.tiles = Object.extend($A([]), {
			refresh: function() {
				this.each(function(col) { col.invoke('assignLayers'); });
				return this;
		 	},
			...
			hide: function() {
				...
				this.each(function(col) { col.each(function(tile) { $(tile.domNode).hide(); }); });
				return this;
			},
			...
			show: function() {
				...
				this.each(function(col) { col.each(function(tile) { $(tile.domNode).show(); }); });
				return this;
			}
	...
	assignTile: function(tile) {
		...
		if (useCache && this.tileCache.contains(this.viewport.zoomLevel, tile.xIndex, tile.yIndex)) {
			...
			this.notifyListeners('AddTileLayer', tile);
			if (useCache) this.tileCache.add(tile.domNode, this.viewport.zoomLevel, tile.xIndex, tile.yIndex);
		}
		return this;
	},
	
	/**
	 * @method getOptimalZoomLevel	Returns the highest possible zoom level at which the complete image still fits in the viewport.
	...
	/**
	 * @method getFullSize	Returns the full size of the tiled image.
	 * @return {Number}		The size.
	 */
	getFullSize: function() {
		Debug.output("getfullsize tiles");
		return this.tileSize * Math.pow(2, this.viewport.zoomLevel);
	},
	...
	/**
	 * TODO: move to tiles
	 * @method reloadTileLayer						Reloads one specific layer of each tile.
	...
	onZoom: function(level, source) {
		Debug.output("onzoom tiles");
		//this.tiles.hide();
		this.positionTiles(null, true);
	 ...
	 * @param {SunViewerWidget} firingWidget	The widget that changed its image.
	 */
	onImageChange: function(sunImage, firingWidget) {
