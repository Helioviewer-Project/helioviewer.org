 ...
 */
 /*global Class, Element, document, Draggable, SunViewerWidget, Event */
var InfoBubble = Class.create();

InfoBubble.prototype = Object.extend({
	/**
	 * @constructor
	...
	createElements: function () {
		...
		//Make draggable
		var trash = new Draggable(this.bubbleLayer);
	...
	observeElements: function () {
		Event.observe(this.closeX, 'click', function () {
		    this.infoBubble.hide();
		    return false;
		});
		Event.observe(this.bubbleLayer, 'mousedown', function (e) {
		    Event.stop(e);
		});
		return this;
	},
	 ...
	 */	
	show: function (id, infoHtml, layer, position) {
	    if ((id === this.lastSeen) && (this.visible === true)) {
	        this.hide();
	    } else {
    		this.position = {
    			x: position.x,
