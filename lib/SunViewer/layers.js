	...
	createTileLayer: function(tile) {
		...
		if (xIndex >= 0
		 ...
		 && yIndex < nxnTiles) {
			...
			loading.className = 'tile';
			loading.setStyle({ 'zIndex': this.zIndex });
			loading.src = this.loadingTileImage;
			...
			var protectionLayer = document.createElement('div');
			protectionLayer.setStyle({ width: '100%', height: '100%' });
			protectionLayer.className = 'tile';
...
Debug.output('Error loading image ' + image.src);
Debug.loadingIndicator.loadingFinished(image.src);
				var div = document.createElement('div');
				div.className = 'noImageAvailable';
				div.innerHTML = self.noImageAvailableMessage;
...
Debug.loadingIndicator.loadingFinished(image.src);
	...
	addTileLayerCached: function(tile) {
		Debug.output("addtilelayer layers");
		var zoomOffset = this.zoomOffset;
	...
	getFullSize: function() {
		Debug.output("getfullsize layers");
	...
	setSunImage: function(sunImage) {
		Debug.output("setsunimage layers");
	...
	setZoomOffset: function(zoomOffset) {
		Debug.output("setzoomoffset layers");
	...
	removeFromTileContainer: function() {
		Debug.output("removefromtilecontainer layers");
	...
	toggleVisible: function(visible) {
		Debug.output("togglevisible layers");
	...
	onAddTileLayer: function(tile) {
		Debug.output("onaddtilelayer layers");
	...
	onImageChange: function(sunImage) {
		Debug.output("onimagechange layers");
/*
...
//Debug.output(this.id);
	...
	onZoomOffsetChange: function(zoomOffset) {
		Debug.output("onzoomoffsetchange layers");
...
OverlayLayerProvider.prototype = Object.extend(new TileLayerProvider(), {
	 ...
	 */
	createTileLayer: function(tile) {
		Debug.output("createtilelayer layers");
		var zoomOffset = this.zoomOffset;
		var zoomLevel = tile.tileContainer.viewport.zoomLevel;
	...
	onNewOverlay: function(overlay) {
		Debug.output("onnewoverlay layers");
...
//Debug.output(zoomLevel, tileIndexOffset, xIndex, yIndex, decimals, tileOffset);
	...
	onShowInfo: function(properties) {
		Debug.output("onshowinfo layers");
...
MarkerLayerProvider.prototype = Object.extend(new OverlayLayerProvider(), {
	 ...
	 */
	initialize: function(tileContainer, options) {
		Debug.output("init markerlayerprovider layers");
		if (tileContainer) {
			Object.extend(this, this.defaultOptions);
			Object.extend(this, options);
...
TileDebugLayerProvider.prototype = Object.extend(new TileLayerProvider(), {
	 ...
	 */
	initialize: function(tileContainer, options) {
		Debug.output("init tiledbuguglayerprovider layers");
		this.color = '#FFFFFF';
	...
	createTileLayer: function(tile) {
		...
		var tileDebugDiv = document.createElement('div');
		tileDebugDiv.className = 'tileDebug';
