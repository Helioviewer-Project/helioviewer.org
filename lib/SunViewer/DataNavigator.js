...
DataNavigator.prototype = {
    ...
    addFilter: function (instrument, wavelength) {
        ...
        var i = this.filters.size() -1;
        this.filters.get(i).clear();
        this.filters.get(i).retreive(null, null, null, this.time, "forward");
        document.sunImageChange.fire(this.filters.get(i).currentSunImage());
    },
    ...
    onNewLayer: function (type, args) {
        ...
        if (layerProvider.type === "TileLayerProvider") {
            this.addFilter("EIT", 171); //Default  Instrument type. Should be set dynamically!
        }
        else {
