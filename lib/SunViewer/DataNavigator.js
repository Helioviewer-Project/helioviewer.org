...
DataNavigator.prototype = {
    ...
    addFilter: function (instrument, wavelength) {
        this.filters.set(this.filters.size(), new Filter(instrument, this.timeIncrement, wavelength));
        var id = this.filters.size() -1;
        this.filters.get(id).clear();
        this.filters.get(id).retreive(null, null, null, this.time, "forward");
        document.sunImageChange.fire(this.filters.get(id).currentSunImage(), id);
    },
    ...
    onNewLayer: function (type, args) {
        ...
        if (layerProvider.type === "TileLayerProvider") {
            ...
            document.sunImageChange.fire(this.filters.get(layerProvider.id).currentSunImage());  //TEMP WORK-AROUND... need to fire again so that marker layers know who to follow (ie. can call setSunImage)
        }
        else {
            this.filters.set (this.filters.size(), null); //To keep indices in sync with layer manager
            document.sunImageChange.fire(this.filters.get(0).currentSunImage());  //TEMP WORK-AROUND... need to fire again so that marker layers know who to follow (ie. can call setSunImage)
        }
    },
    ...
    setTime: function (year, month, day, hour, minute, second) {
        ...
        this.filters.each(function (filter, id) {
            if (filter.value != null) {
                filter.value.clear();       
                filter.value.retreive(null, null, null, time, "forward");
                document.sunImageChange.fire(filter.value.currentSunImage(), id); //TODO: Check to see if image differs from current!
            }
        });      
    },
    ...
    timePrevious: function () {
        ...
        var self = this;
	    this.filters.each(function (filter, id) {
	        if ((filter.value != null) && filter.value.previous(self.time)) {
	            document.sunImageChange.fire(filter.value.currentSunImage(), id);
	        }
	    });
	},
	...
	timeNext: function () {
	    ...
	    this.filters.each(function (filter, id) {
	        if ((filter.value != null) && filter.value.next(self.time)) {
	            document.sunImageChange.fire(filter.value.currentSunImage(), id);
