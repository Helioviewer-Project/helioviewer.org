...
DataNavigator.prototype = {
    ...
    timePrevious: function () {
        ...
        if (this.time <= Date.parse(DATE_START_EIT)) {
            ...
            trash = new Effect.Shake('viewport-outer', {distance:15, duration:0.1});
            var trash = new Effect.Appear('message-console', { duration: 3.0 });
        
            //Hide the message after several seconds have passed
            window.setTimeout(function() {var trash = new Effect.Fade('message-console', { duration: 3.0 });},6500);
