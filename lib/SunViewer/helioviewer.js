var Helioviewer = Class.create({
  defaultOptions: {
    ...
    mapUrlPrefix: 'getMap.php'
  },
  date: new Date(Date.UTC(2003,9,02)),
  sources: {
    events: {
      noaa: { enabled: true },
      goes: { enabled: true },
  ...
  setDate: function(date) {
    this.date = date;
    this.viewports.each(function(viewport) { viewport.reload(); });
    this.calendar.updateFields();
  },
  
  handlers: {
    zoom: function(level) {
      this.viewports.each(function(viewport) { viewport.zoomTo(level); });
    },
