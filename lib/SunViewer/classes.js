...
var UrlTileLayer = Class.create(Layer, {
  ...
  setMapProperties: function(mapProperties) {
    if (mapProperties.map == this.map) return;
    Object.extend(this, mapProperties);
    this.fire('change', this);
    this.resetTiles();
  },

  loadMapProperties: function() {
    var urlPrefix = this.viewport.controller.mapUrlPrefix;
    var url = urlPrefix + '?action=getProperties&map=' + this.map;
    
    var processResponse = function(transport) {
      this.setMapProperties(transport.responseJSON);
    };
  ...
  getTileUrl: function(x, y, zoom, map) {
    if (map == undefined) map = '';
    return this.tileUrlPrefix + '?x=' + x + '&y=' + y + '&zoom=' + zoom + '&map=' + map;
  },
...
//      height: '256px',
    ...
    img.src = this.getTileUrl(x, y, this.viewport.zoomLevel, this.map);
    //img.innerHTML = x + ',' + y;
    //div.appendChild(img);
    //return div;
    return img;
  ...
  reload: function() {
//alert('reloading ' + this.instrument + ' to ' + this.map);
    this.loadClosestMap();
    //if (this.map) this.resetTiles();
  }
});
