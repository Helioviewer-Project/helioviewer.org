    ...
    window.boneVojage_main = {
      ...
      constructor: function(points, options) {
        ...
        if (boneVojage_main.options.buttons) {
          buttons = '<div id="buttons">' + '<div id="prev">&larr; prev</div>' + '<div id="next">next &rarr;</div>' + '<div id="close_btn" style="display:none">Finish</div>' + '</div>';
        } else {
          buttons = '';
        }
          ...
          return boneVojage_main.settings.$tooltip.on('click', '#next', function() {
            ...
            return boneVojage_main.settings.$body_html.animate({
              ...
              //'scrollLeft': boneVojage_main.settings.scrollBeforeVojageX
	            ...
	            boneVojage_main.settings.$tooltip.hide().remove();
		          boneVojage_main.settings.$modalTop.hide().remove();
		          boneVojage_main.settings.$modalBottom.hide().remove();
		          boneVojage_main.settings.$modalLeft.hide().remove();
		          boneVojage_main.settings.$modalRight.hide().remove();
            });
      ...
      O: {
          ...
          switch (pos) {
            ...
            case 'top':
              boneVojage_main.settings.$tooltip.css({
                ...
                left: $el.offset().left + ($el.outerWidth() / 2) - (boneVojage_main.settings.$tooltip.outerWidth() / 2) - norm.normY - 100
              });
              break;
            case 'top_left':
              boneVojage_main.settings.$tooltip.css({
        ...
        addControls: function() {
          ...
          if(boneVojage_main.settings.currPoint == boneVojage_main.settings.points.length - 1){
	          boneVojage_main.settings.$tooltip.find('#close_btn').show();
          }else{
	          boneVojage_main.settings.$tooltip.find('#close_btn').hide();
          }
          if (boneVojage_main.settings.currPoint > 0) {
        ...
        makeModalPosition: function(element) {
          ...
          boneVojage_main.settings.$body_html.stop(true, false).animate({
            ...
            if (cnt++ === 0) {
              dfr.resolve();
              if($el.selector == '#moving-container'){
              ...
              boneVojage_main.settings.$tooltip.find('#text').html(boneVojage_main.settings.points[boneVojage_main.settings.currPoint].text);
              var positionP = boneVojage_main.settings.points[boneVojage_main.settings.currPoint].position;
              boneVojage_main.O.addControls();
              return boneVojage_main.O.makeTooltipPosition(positionP, $el, offset);
            }
          });
          return dfr.promise();
        }
