...
test('GetJhvRequestForMovie can extract layers from layer strings', async () => {
    ...
    let KATs = [
        {
            send: false,
            layerString: "[SOHO,LASCO,C2,white-light,2,100,0,60,1,2022-10-03T15:51:26.000Z],[SOHO,LASCO,C3,white-light,3,100,0,60,1,2022-10-03T15:51:26.000Z],[SDO,AIA,304,1,100,0,60,1,2022-10-03T15:51:26.000Z]",
            expectedResult: [{
                observatory: "SOHO",
                dataset: "LASCO C2"
            }, {
...
function ValidateRequest(result, expected, layerString) {
    ...
    for (const expectedLayer of expected) {
        ...
        if (typeof foundLayer === "undefined") {
            console.error(`Didn't find expected layer ${expectedLayer.observatory} ${expectedLayer.dataset} in layerString: ${layerString}`);
            console.log(layers);
        }
