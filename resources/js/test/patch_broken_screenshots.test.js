...
const { _BrokenScreenshotNotifier } = require("../Patches/broken_screenshots_493.js");
...
describe("Broken screenshot patch", () => {
  it("Should count the number of broken screenshots in the list", () => {
    ...
    Helioviewer.userSettings.get.mockReturnValueOnce([{ id: 1 }, { id: false }]);
    ...
    Helioviewer.userSettings.get.mockReturnValueOnce([{ id: false }, { id: false }, { id: false }]);
    patch = new _BrokenScreenshotNotifier();
    expect(patch.num_broken_screenshots).toBe(3);

    // Repeat the test with no invalid screenshots
    Helioviewer.userSettings.get.mockReturnValueOnce([{ id: 0 }, { id: 1 }, { id: 2 }]);
    patch = new _BrokenScreenshotNotifier();
  ...
  it("Should remove broken screenshots from the history list", () => {
    ...
    Helioviewer.userSettings.get.mockReturnValueOnce([{ id: false }, { id: false }, { id: false }, { id: 1337 }]);
    let patch = new _BrokenScreenshotNotifier();
    patch.Apply();
    ...
    expect(Helioviewer.userSettings.set.mock.calls[0][0]).toBe("history.screenshots");
    expect(Helioviewer.userSettings.set.mock.calls[0][1]).toStrictEqual([{ id: 1337 }]);
  });
  ...
  it("Should notify the user that N items were removed from their screenshot list", () => {
    ...
    expect($().trigger.mock.calls[0][1][0]).toContain("3 of your recent screenshots");
  });
});
