...
test("[Mobile] Center viewport with AIA 304", async ({ page }, info) => {
  ...
  const opts: PageAssertionsToHaveScreenshotOptions = {
 ...
 */
test("[Mobile] Center viewport with AIA 304 and LASCO C2/C3", async ({
  page,
}, info) => {
  let mobile = new HvMobile(page, info);
  await mobile.Load();
 ...
 */
test("[Mobile] Switch between earth scale, bar scale, and no scale", async ({
  page,
}) => {
 ...
 */
test("[Mobile] Verify earth/bar indicator scales with zoom", async ({
  page,
}) => {
  let mobile = new HvMobile(page);
  await mobile.Load();
