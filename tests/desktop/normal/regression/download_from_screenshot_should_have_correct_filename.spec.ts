 ...
 */
test("Download from screenshot fullscreen view, should correctly set filename", async ({
  page,
  browserName,
}) => {
  test.fixme(
  ...
  const download =
    await hv.screenshot.fetchScreenshotFromViewScreenshotFeature();

  // Expect the filename matches with our screenshot filenames
  await expect(download.suggestedFilename()).toMatch(
    /\d{4}_\d{2}_\d{2}_\d{2}_\d{2}_\d{2}_AIA_304\.png/,
  );
});
