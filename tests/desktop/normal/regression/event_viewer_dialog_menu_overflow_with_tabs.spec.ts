...
test("Event viewer dialog menu should not overflow with tabs", async ({
  page,
}) => {
  ...
  await expect(
    page.locator("#tree_CCMC").getByRole("link", { name: "CME" }),
  ).toBeVisible();
  ...
  await page.evaluate(() => {
    $(".event-info-dialog-menu").append(
      $("div.tabgroup:nth-of-type(2)").clone(),
    );
    $(".event-info-dialog-menu").append(
      $("div.tabgroup:nth-of-type(2)").clone(),
    );
  ...
  expect(await dialogMenu.evaluate((a) => a.scrollWidth > a.clientWidth)).toBe(
    true,
  );
});
            ...
            views: [
              {
                ...
                content: {
                  ...
                  Longitude: 78,
                  "External Link":
                    "https://kauai.ccmc.gsfc.nasa.gov/DONKI/view/CME/17028/-1",
                  Instruments:
                    "SOHO: LASCO/C2, SOHO: LASCO/C3, STEREO A: SECCHI/COR2",
                  "Related Events": null,
                  Note: "",
                },
              },
