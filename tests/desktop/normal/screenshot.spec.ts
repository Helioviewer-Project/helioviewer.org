...
test("Screenshot button should toggle screenshot drawer", async ({
  page,
  context,
}, info) => {
  ...
  expect(await page.screenshot()).toMatchSnapshot(
    "screenshot-drawer-enabled.png",
  );

  await hv.screenshot.toggleScreenshotDrawer();

  expect(await page.screenshot()).toMatchSnapshot(
    "screenshot-drawer-disabled.png",
  );
});

// create multiple screenshots and compare them
test("Create a new screenshot and view it and close it", async ({
  page,
  context,
  browserName,
}, info) => {
  ...
  const screenshotMask: PageScreenshotOptions = {
    mask: [page.locator("#screenshot-manager-container .status")],
  };
  ...
  const screenshotFileFromNotification =
    await hv.screenshot.downloadScreenshotFromNotification();
  const screenshot_notification_report_file = info.outputPath(
    "screenshot_from_notification.png",
  );
  await fs.promises.writeFile(
    screenshot_notification_report_file,
    Buffer.from(screenshotFileFromNotification, "base64"),
  );
  await info.attach("screenshot-notification", {
    path: screenshot_notification_report_file,
  });

  // close the notification
  await hv.CloseAllNotifications();

  expect(await page.screenshot(screenshotMask)).toMatchSnapshot(
    "first-screenshot-created.png",
  );
  ...
  const screenshotFileFromScreenshotView =
    await hv.screenshot.downloadScreenshotFromViewScreenshotFeature();
  const screenshot_view_screenshot_report_file = info.outputPath(
    "screenshot_view_screenshot.png",
  );
  await fs.promises.writeFile(
    screenshot_view_screenshot_report_file,
    Buffer.from(screenshotFileFromScreenshotView, "base64"),
  );
  await info.attach("screenshot-view-screenshot", {
    path: screenshot_view_screenshot_report_file,
  });

  // compare screenshots downloaded from different sources
  await expect(screenshotFileFromNotification).toBe(
    screenshotFileFromScreenshotView,
  );
