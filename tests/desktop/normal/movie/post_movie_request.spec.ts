...
[
  ...
  test(
    'Movie range "' + label + '" should correctly rely to API in Requests',
    async ({ page }) => {
...
[{ frame_per_sec: 15 }, { frame_per_sec: 25 }].forEach(({ frame_per_sec }) => {
  test(
    'Set frame per second : "' +
      frame_per_sec +
      '" should correctly rely to API in Requests',
    async ({ page }) => {
      let hv = new Helioviewer(page);
      ...
      await page
        .getByRole("spinbutton", { name: "Frames/Sec" })
        .fill(frame_per_sec.toString());
      ...
      expect(movieJSON).toEqual(
        expect.objectContaining({ frameRate: frame_per_sec }),
      );
    },
  );
});
...
[{ length: 20 }, { length: 40 }].forEach(({ length }) => {
  test(
    'Set length : "' + length + '" should correctly rely to API in Requests',
    async ({ page }) => {
      let hv = new Helioviewer(page);
      ...
      await page
        .getByRole("spinbutton", { name: "Length (s)" })
      ...
      expect(movieJSON).toEqual(
        expect.objectContaining({ movieLength: length }),
      );
    },
  );
});

test("Display youtube movies icons checkbox sould correctly rely to API in Requests", async ({
  page,
}) => {
  let hv = new Helioviewer(page);
