...
test("If there is not enough data, helioviewer should show error message about no data situation", async ({
  page,
}) => {
  ...
  await hv.assertNotification(
    "error",
    "No images found for requested time range. Please try a different time.",
  );
});
