...
time_jump_ranges.forEach(({ jump_label, seconds }) => {
  ...
  test(
    "Jump backwards with " +
      jump_label +
      " should go to matching datetime in past, with matching screenshots",
    { tag: "@production" },
    async ({ page, context, browser }, info) => {
      ...
      await expect(seconds * 1000).toBe(
        dateBeforeJump.getTime() - dateAfterJump.getTime(),
      );

      // 8. SAVE CURRENT SCREENSHOT TO COMPARE LATER
      const afterJumpScreenshot = await hv.saveScreenshot(
        "after_jump_screenshot",
        {
  ...
  test(
    "Jump forwards with " +
      jump_label +
      " should go to matching datetime in future, with matching screenshots",
    { tag: "@production" },
    async ({ page, context, browser }, info) => {
      const hv = new Helioviewer(page, info);
      ...
      wayBackInTime.setTime(
        initialDate.getTime() - seconds * 1000 - randomMilliseconds,
      );
      ...
      await expect(randomMilliseconds).toBe(
        initialDate.getTime() - dateAfterJumpForward.getTime(),
      );

      // 8. TAKE A PICTURE , WE WILL COMPARE LATER
      const navigatedDateScreenshot = await hv.saveScreenshot(
        "navigated_date_screenshot",
        {
