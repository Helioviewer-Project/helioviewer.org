...
test("Pressing zoom out button should zoom sun out", async ({ page }, info) => {
  ...
  // 4. TAKE A SCREENSHOT
  await hv.sunScreenshot("zoom_one_out_with_button_screenshot.png");
  ...
  const zoomOneOutWithURL = await hv.sunScreenshot("zoom_one_out_with_url_screenshot");

  // 8. BOTH SCREENSHOT SHOULD MATCH
  expect(Buffer.from(zoomOneOutWithURL, "base64")).toMatchSnapshot("zoom_one_out_with_button_screenshot.png");
});
...
test("Pressing zoom out with keyboard + should zoom sun out", async ({ page }, info) => {
  ...
  // 4. TAKE A SCREENSHOT
  await hv.sunScreenshot("zoom_one_out_with_keyboard_screenshot.png");
  ...
  // 8. BOTH SCREENSHOT SHOULD MATCH
  expect(Buffer.from(zoomOneOutWithURL, "base64")).toMatchSnapshot("zoom_one_out_with_keyboard_screenshot.png");
});
