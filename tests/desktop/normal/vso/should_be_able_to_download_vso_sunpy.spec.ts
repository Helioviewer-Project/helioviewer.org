 ...
 */
test.skip("Validate sunpy ssw download button brings notification with dowload link", async ({
  page,
}) => {
  ...
  await hv.assertNotification(
    "success",
    "Your Python/SunPy script for requesting science data from the VSO is ready",
  );
});
 ...
 */
test.skip("Validate sunpy ssw download link downloads sunpy script", async ({
  page,
  browserName,
}) => {
  ...
  expect(sunpyFile).toContain(
    "result_aia_304 = Fido.search(a.Time(tstart, tend)",
  );
});
