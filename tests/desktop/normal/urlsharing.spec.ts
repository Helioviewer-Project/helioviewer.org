...
test("Non-Existing Shared URLs Should Be Reported To User", async ({
  page,
  context,
}, info) => {
 ...
 */
test("Shared URLs redirection should be done correctly", async ({
  page,
}, info) => {
  let hv = new Helioviewer(page);
 ...
 * This test plays through creating a short url and makes a test for short url should exactly resolved what is shared
 */
test("Shared URLs should produce pages, exactly like they shared", async ({
  page,
  browserName,
}, info) => {
  test.fixme(
  ...
  await expect(
    page.locator("#helioviewer-url-box-stale-link-msg"),
  ).not.toBeVisible();
  ...
  await expect(
    page.locator("#helioviewer-url-box-stale-link-success-msg"),
  ).toBeVisible();
  ...
  await fs.promises.writeFile(
    after_file,
    Buffer.from(afterScreenshot, "base64"),
  );
