 ...
 *   5. Share Viewport
 */
test("Verify that you can share state after setting baseDiffTime", async ({
  page,
}) => {
  // 0. Load page
  let hv = new Helioviewer(page);
 ...
 */
test("Verify that you can share state after upgrading from old baseDiffDate format", async ({
  page,
}) => {
  // 1. Load page
  let hv = new Helioviewer(page);
