...
[MobileView, DesktopView].forEach((view) => {
   ...
   *   5. Share Viewport
   */
  test(`[${view.name}] Verify that you can share state after setting baseDiffTime`, {tag: view.tag}, async ({ page }, info) => {
    // 0. Load page
    let hv = HelioviewerFactory.Create(view, page, info) as MobileInterface;
    await hv.Load();
    await hv.CloseAllNotifications();
   ...
   */
  test(`[${view.name}] Verify that you can share state after upgrading from old baseDiffDate format`, {tag: view.tag}, async ({ page }, info) => {
    // 1. Load page
    let hv = HelioviewerFactory.Create(view, page, info) as MobileInterface;
    await hv.Load();
