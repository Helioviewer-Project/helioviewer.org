...
HelioviewerViews.forEach((view) => {
  ...
  test(`[${view}] Verify image tiles are loaded when the viewport pans to tile boundaries after zooming in and out`, async ({
    ...
    await expect(page.locator("//img[contains(@src, 'x=1&y=-2')]")).toHaveCount(
      1,
    );
    await expect(page.locator("//img[contains(@src, 'x=1&y=-1')]")).toHaveCount(
      1,
    );
    await expect(page.locator("//img[contains(@src, 'x=1&y=0')]")).toHaveCount(
      1,
    );
    await expect(page.locator("//img[contains(@src, 'x=1&y=1')]")).toHaveCount(
      1,
    ...
    await expect(page.locator("//img[contains(@src, 'x=2&y=-2')]")).toHaveCount(
      1,
    );
    await expect(page.locator("//img[contains(@src, 'x=2&y=-1')]")).toHaveCount(
      1,
    );
    await expect(page.locator("//img[contains(@src, 'x=2&y=0')]")).toHaveCount(
      1,
    );
    await expect(page.locator("//img[contains(@src, 'x=2&y=1')]")).toHaveCount(
      1,
    ...
    await expect(page.locator("//img[contains(@src, 'x=-3&y=1')]")).toHaveCount(
      1,
    );
    await expect(page.locator("//img[contains(@src, 'x=-2&y=1')]")).toHaveCount(
      1,
    ...
    await expect(page.locator("//img[contains(@src, 'x=0&y=1')]")).toHaveCount(
      1,
    );
    await expect(page.locator("//img[contains(@src, 'x=1&y=1')]")).toHaveCount(
      1,
