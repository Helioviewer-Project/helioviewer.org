...
test("[Normal] Helioviewer shows a warning when the image displayed is at least 6 hours away from the observation time.", async ({
  ...
  await expect(
    await page.getByText("The AIA 304 layer is 6 hours").count(),
  ).toBe(1);
  ...
  await expect(
    page.getByText("The AIA 304 layer is 6 hours"),
  ).not.toBeVisible();
});
...
test("[Minimal] Helioviewer shows a warning when the image displayed is at least 6 hours away from the observation time.", async ({
  ...
  await expect(
    await page.getByText("The AIA 171 layer is 6 hours").count(),
  ).toBe(1);
  ...
  await expect(
    page.getByText("The AIA 171 layer is 6 hours"),
  ).not.toBeVisible();
});
