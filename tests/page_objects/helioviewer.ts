...
class Helioviewer {
  ...
  async ExpectLayerEx(index: number, name: string, selections: LayerSelect[]) {
    ...
    for (let i = 0; i < selections.length; i++) {
      ...
      await expect(this.page.getByLabel(layer.label).nth(index)).toHaveValue(
        layer.value,
      );
    }
  }
  ...
  async AddImageLayer() {
    ...
    await this.WaitForLoadingComplete();
    await expect(this.page.locator(".removeBtn")).toHaveCount(
      initial_count + 1,
    );
    await this.WaitForLoadingComplete();
  }
  ...
  async IsSidebarOpen(): Promise<boolean> {
    let width = (
      await this.sidebar.evaluate((html) => html.style.width)
    ).trim();
  ...
  async assertNotification(type: string, message: string) {
    await expect(
      this.page
        .locator("div.jGrowl-notification." + type + " > div.jGrowl-message")
        .getByText(message),
    ).toBeVisible();
  }
  ...
  async GetLoadedDate(): Promise<Date> {
    const currentDate = await this.page
      ...
      .inputValue();
    const currentTime = await this.page
      .getByLabel("Observation time", { exact: true })
      .inputValue();
