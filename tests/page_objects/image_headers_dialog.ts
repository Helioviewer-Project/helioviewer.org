...
class ImageHeaderDialog {
  ...
  async assertTabs(tags: Array<string>, selected: string): Promise<void> {
    for (let t of tags) {
      await expect(
        this.box.locator(".image-info-dialog-menu").getByText(t),
      ).toBeVisible();
    }

    await expect(
      this.box
        .locator(".image-info-dialog-menu")
        .getByText("[" + selected + "]"),
    ).toBeVisible();
  }
   ...
   * @return void
   */
  async assertImageHeaders(
    imageHeaders: Array<[string, string]>,
  ): Promise<void> {
    await Promise.all(
      imageHeaders.map(async (_, index) => {
        const headerTag = await this.box
