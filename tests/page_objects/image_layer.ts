...
class ImageLayer {
  ...
  constructor(page: Page, id: string) {
    this.page = page;
    this.id = id;
    this.opacity_slider = this.page.locator(
      "#opacity-slider-track-tile-layer-" + id,
    );
    this.opacity_slider_handle =
      this.opacity_slider.locator(".ui-slider-handle");
  ...
  async set(label: string, value: string) {
    let selection = await this.layer_controls.getByLabel(label, {
      exact: true,
    });
  ...
  async setRunningDifferenceValue(value: number) {
    ...
    let input = this.layer_controls.getByLabel("Running difference", {
      exact: true,
    });
  ...
  async getBaseDifferenceTime(): Promise<string> {
    return await this.layer_controls
      .getByLabel("Time", { exact: true })
      .inputValue();
  }
  ...
  getTile(index: number): Locator {
    return this.page
      .locator(`.tile-layer-container[rel=tile-layer-${this.id}] .tile`)
      .nth(index);
  }
  ...
  async assertImageDateAvailable(): Promise<void> {
    await expect(this.layer_controls.locator(".timestamp")).toHaveCSS(
      "color",
      "rgb(0, 255, 0)",
    );
  }
  ...
  async assertHasNextImage(): Promise<void> {
    await expect(this.layer_controls.locator(".next-image-btn")).toHaveCSS(
      "cursor",
      "pointer",
    );
    await expect(this.layer_controls.locator(".next-image-btn")).toHaveCSS(
  ...
  async assertHasPreviousImage(): Promise<void> {
    await expect(this.layer_controls.locator(".prev-image-btn")).toHaveCSS(
      "cursor",
      "pointer",
    );
    await expect(this.layer_controls.locator(".prev-image-btn")).toHaveCSS(
  ...
  async downloadJp2(): Promise<Download> {
    ...
    await this.layer_controls
      .getByTitle("Download full JPEG 2000 image (grayscale).")
      .click();
  ...
  async getImageHeaderDialog(): Promise<ImageHeaderDialog> {
    const box = await this.page.locator(
      "//div[contains(@aria-describedby, 'image-info-dialog-tile-layer')]",
    );
