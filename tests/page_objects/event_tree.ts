...
class EventTree {
  ...
  async hasEventType(event_type: string): Promise<boolean> {
    ...
    const eventTypeCount = await this.root
      .getByRole("listitem")
      .filter({ has: eventTypeLink })
      .count();
  ...
  async hasFRM(event_type: string, frm: string): Promise<boolean> {
    const eventTypeLink = this.page.getByRole("link", { name: event_type });
    const eventTypeNode = await this.root
      .getByRole("listitem")
      .filter({ has: eventTypeLink });

    const eventFRMLink = this.page.getByRole("link", { name: frm });
    const eventFRMNodeCount = await eventTypeNode
      .getByRole("listitem")
      .filter({ has: eventFRMLink })
      .count();
  ...
  async frmEventCount(event_type: string, frm: string): Promise<number> {
    ...
    const eventFRMLink = this.page.getByRole("link", { name: frm });
    const eventFRMNode = await eventTypeNode
      .getByRole("listitem")
      .filter({ has: eventFRMLink });

    return await eventFRMNode
  ...
  async frmHasEventInstance(
    ...
    const eventInstanceLink = this.page.getByRole("link", {
      name: event_instance,
      includeHidden: true,
    });
  ...
  async toggleCheckEventType(event_type: string) {
    await this.root
      .getByRole("link", { name: event_type })
      .getByRole("insertion")
      .first()
      .click();
  }
   ...
   * @return void promise about the task is done
   **/
  async toggleCheckEventInstance(
    event_type: string,
    frm: string,
    event_instance: string,
  ) {
    ...
    await eventFRMNode
      .getByRole("link", { name: event_instance })
      .getByRole("insertion")
      .first()
      .click();
  }
   ...
   **/
  async assertEventInstanceTreeNodeVisible(
    event_type: string,
    frm: string,
    event_instance: string,
  ) {
    ...
    expect(
      eventFRMNode.getByRole("link", { name: event_instance }),
    ).toBeVisible();
  }
   ...
   * @return void promise about the assertion is done
   **/
  async assertEventInstanceTreeNodeNotVisible(
    event_type: string,
    frm: string,
    event_instance: string,
  ) {
