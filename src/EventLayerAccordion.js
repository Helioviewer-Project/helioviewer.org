...
/*global $, EventLayer, IconPicker, Layer */
"use strict";
var EventLayerAccordion = Layer.extend(
    ...
    addLayer: function (layer) {
        ...
        icon = "<span class=accordion-header-divider>|</span><button class='event-accordion-icon' id='event-icon-" + 
               layer.id + "' title='Select an icon to use for this event' style='background: " +
               "url(resources/images/events/"  + layer.icon + "-" + etype + ".png);'></button>";
        visibilityBtn = "<span class='layerManagerBtn visible' id='visibilityBtn-" + layer.id +
                        "' title='Toggle layer visibility'></span>";
        removeBtn = "<span class='ui-icon ui-icon-closethick removeBtn' id='removeBtn-" + layer.id +
                    "' type=button title='Remove layer'></span>";
        head = "<div class='layer-Head ui-accordion-header ui-helper-reset ui-state-default ui-corner-all'>" +
               "<span class=event-accordion-header-left>" + catalog.name +
               "</span><span class=event-accordion-header-right>" + icon + visibilityBtn + removeBtn + "</span></div>";
        ...
        addLayerBtn.click(function () {
            if (typeof(self.eventCatalogs) !== "undefined") {
                ...
                Event.observe($(tmpId).select('button').first(), 'click', function (e) {
                    ...
                    self.eventLayers.addLayer(new EventLayer(self.controller, {
    ...
    _buildCatalogSelect: function () {
        ...
        for (catalog in this.eventCatalogs) {
    ...
    _catalogsUnavailable: function () {
        ...
        // Error message
        var head = "<div class='layer-Head ui-accordion-header ui-helper-reset ui-state-default ui-corner-all'>" +
    ...
    _setupTooltips: function (id) {
        var selector = $("#event-icon-" + id + ", #visibilityBtn-" + id + ", #removeBtn-" + id);
        this.controller.tooltips.createTooltip(selector);
    },
    ...
    _removeTooltips: function (id) {
        $("#event-icon-" + id + ", #visibilityBtn-" + id + ", #removeBtn-" + id).qtip("destroy");
    },
    ...
    _setupEventHandlers: function (layer) {
        ...
        toggleVisibility = function (e) {
            var visible = layer.toggleVisibility(),
                icon = (visible ? 'LayerManagerButton_Visibility_Visible.png' : 
                                  'LayerManagerButton_Visibility_Hidden.png');
            $("#visibilityBtn-" + layer.id).css('background', 'url(resources/images/blackGlass/' + icon + ')');
            e.stopPropagation();
        },
        ...
        showIconMenu = function (e) {
            layer = e.data;
            $(".event-accordion-icon").qtip("hide");
